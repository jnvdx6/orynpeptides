  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-oryn-orange/5 border border-oryn-orange/15 p-6">
        <h3 className="text-[10px] font-mono text-oryn-orange tracking-[0.2em] mb-4">{r.yourCode}</h3>
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          >
            {copied === "code" ? r.copied : r.copyCode}
          </button>
        </div>
        ...
        <div className="flex items-center gap-3">
          <span className="text-[9px] font-mono text-oryn-black/30 tracking-[0.1em]">{r.shareVia}</span>
          <button
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: r.directReferrals, value: stats?.directReferrals || 0, format: "number" },
          { label: r.networkSize, value: stats?.totalNetworkSize || 0, format: "number" },
          { label: r.totalEarned, value: stats?.totalEarnings || 0, format: "currency" },
          { label: r.available, value: data?.availableBalance || 0, format: "currency" },
        ].map((stat) => (
      ...
      {stats?.commissionsByLevel && stats.commissionsByLevel.length > 0 && (
        <div className="bg-white border border-oryn-grey/15 p-6">
          <h3 className="text-[10px] font-mono text-oryn-orange tracking-[0.2em] mb-4">{r.earningsByLevel}</h3>
                ...
                <div key={level} className="flex items-center justify-between px-4 py-3 bg-oryn-grey-light">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs font-plex text-oryn-black/50">
                      {r.levelLabel} {level} ({rates[level - 1]}% {r.commission})
                    </span>
                  </div>
                  <div className="text-right">
                    ...
                    <span className="text-[9px] text-oryn-black/30 font-plex ml-2">
                      ({levelData?.count || 0} {r.ordersLabel})
                    </span>
                  </div>
                </div>
              );
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-yellow-50 border border-yellow-200 p-4">
          <span className="text-[9px] font-mono text-yellow-600 tracking-[0.15em]">{r.pending}</span>
          <p className="text-xl font-bold text-yellow-700 mt-1">{formatPrice(stats?.pendingCommissions || 0)}</p>
        </div>
        <div className="bg-green-50 border border-green-200 p-4">
          <span className="text-[9px] font-mono text-green-600 tracking-[0.15em]">{r.approved}</span>
          <p className="text-xl font-bold text-green-700 mt-1">{formatPrice(stats?.approvedCommissions || 0)}</p>
        </div>
        <div className="bg-blue-50 border border-blue-200 p-4">
          <span className="text-[9px] font-mono text-blue-600 tracking-[0.15em]">{r.paidOut}</span>
      ...
      {data?.directReferrals && data.directReferrals.length > 0 && (
        <div className="bg-white border border-oryn-grey/15 p-6">
          <h3 className="text-[10px] font-mono text-oryn-orange tracking-[0.2em] mb-4">{r.yourReferrals}</h3>
      ...
      {data?.commissions && data.commissions.length > 0 && (
        <div className="bg-white border border-oryn-grey/15 p-6">
          <h3 className="text-[10px] font-mono text-oryn-orange tracking-[0.2em] mb-4">{r.commissionHistory}</h3>
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-oryn-grey/10">
                  <th className="text-left py-2 font-mono text-[9px] text-oryn-black/30 tracking-[0.1em]">{r.tableOrder}</th>
                  <th className="text-left py-2 font-mono text-[9px] text-oryn-black/30 tracking-[0.1em]">{r.tableLevel}</th>
                  <th className="text-left py-2 font-mono text-[9px] text-oryn-black/30 tracking-[0.1em]">{r.tableRate}</th>
                  <th className="text-right py-2 font-mono text-[9px] text-oryn-black/30 tracking-[0.1em]">{r.tableAmount}</th>
                  <th className="text-right py-2 font-mono text-[9px] text-oryn-black/30 tracking-[0.1em]">{r.tableStatus}</th>
                  <th className="text-right py-2 font-mono text-[9px] text-oryn-black/30 tracking-[0.1em]">{r.tableDate}</th>
                </tr>
              </thead>
      ...
      {/* How it works */}
      <div className="bg-white border border-oryn-grey/15 p-6">
        <h3 className="text-[10px] font-mono text-oryn-orange tracking-[0.2em] mb-4">{r.howItWorks}</h3>
