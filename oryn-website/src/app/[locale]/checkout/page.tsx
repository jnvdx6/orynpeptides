...
type CheckoutStep = "contact_delivery" | "shipping_payment";
...
function getEstimatedDelivery(optionName: string): string | null {
  const match =
    optionName.match(/(\d+)\s*-\s*(\d+)\s*business\s*days/i) ||
    optionName.match(/(\d+)\s*-\s*(\d+)\s*d[ií]as\s*h[aá]biles/i);
...
function ConfettiCanvas() {
  ...
  useEffect(() => {
    ...
    const colors = ["#FF6A1A", "#FFB347", "#1a1a1a", "#16a34a", "#3b82f6", "#a855f7"];
...
const INPUT_BASE =
  "w-full px-4 py-3 bg-oryn-grey-light/50 border text-[16px] sm:text-sm focus:outline-none focus:border-oryn-orange transition-colors";

function inputCls(hasError: boolean) {
  return `${INPUT_BASE} ${hasError ? "border-red-300" : "border-oryn-grey/30"}`;
}
...
export default function CheckoutPage() {
  ...
  // ─── Steps ──────────────────────────────────────────────────────────
  const [activeStep, setActiveStep] = useState<CheckoutStep>("contact_delivery");
  const [completedSteps, setCompletedSteps] = useState<Set<CheckoutStep>>(new Set());

  const stepKeys: CheckoutStep[] = ["contact_delivery", "shipping_payment"];
  const stepLabels = [
    t.checkoutPage.steps?.[0] || "Contact & Delivery",
    t.checkoutPage.steps?.[1] || "Shipping & Payment",
  ];

  // ─── Form: contact & shipping ───────────────────────────────────────
  ...
  // ─── Validation ─────────────────────────────────────────────────────
  const validateContactDelivery = (): boolean => {
    const errs: Record<string, string> = {};
    const req = t.checkoutPage.required;

    if (!email.trim() || !/\S+@\S+\.\S+/.test(email))
      errs.email = t.checkoutPage.validEmailRequired;
    ...
    if (!billingSameAsShipping) {
      if (!billingFirstName.trim()) errs.billingFirstName = req;
      if (!billingLastName.trim()) errs.billingLastName = req;
      if (!billingAddress.trim()) errs.billingAddress = req;
      if (!billingCity.trim()) errs.billingCity = req;
      if (!billingPostalCode.trim()) errs.billingPostalCode = req;
      if (!billingCountry) errs.billingCountry = req;
    }
  ...
  const handleContinueToPayment = async () => {
    if (!validateContactDelivery()) return;
    ...
    try {
      if (medusaConnected) {
        ...
        if (!billingSameAsShipping && cart) {
          const billingAddr = {
            first_name: billingFirstName,
            last_name: billingLastName,
            address_1: billingAddress + (billingApartment ? `, ${billingApartment}` : ""),
            city: billingCity,
            postal_code: billingPostalCode,
            country_code: billingCountry?.toLowerCase() || "gb",
          };
          try {
            ...
            console.error("[ORYN Checkout] Billing address update failed:", err instanceof Error ? err.message : err);
          }
        }
      ...
      setCompletedSteps(new Set(["contact_delivery"]));
      setActiveStep("shipping_payment");
      window.scrollTo({ top: 0, behavior: "smooth" });
      trackCheckoutStep("shipping", {
        item_count: totalItems,
        total: totalPrice,
      });
      trackPaymentInfoEntered("stripe");
    } catch (err) {
      console.error("[ORYN Checkout] Continue failed:", err instanceof Error ? err.message : err);
    } finally {
  ...
  const handlePaymentSuccess = (orderId: string, ref: string) => {
    setOrderRef(ref);
    setOrderTotal(cart?.total != null ? cart.total : finalTotal);
    setOrderItemsCount(totalItems);
    trackPurchase({
      orderId,
      orderRef: ref,
  ...
  // ─── Handle shipping selection in step 2 ────────────────────────────
  const handleShippingSelect = async (optionId: string) => {
    if (optionId === selectedShipping) return;
    setSelectedShipping(optionId);
    if (medusaConnected && cart && optionId !== "free_shipping") {
      try {
        await addShippingMethod(optionId);
      } catch (err) {
        console.error("[ORYN Checkout] Shipping select failed:", err instanceof Error ? err.message : err);
      }
    }
  };
  ...
  // ═══════════════════════════════════════════════════════════════════
  // RENDER: Order confirmation (inline success with confetti)
  // ═══════════════════════════════════════════════════════════════════
    ...
    return (
      <div className="pt-24 min-h-screen flex items-center justify-center px-6">
        ...
        <div className="text-center max-w-md relative z-10">
          {/* Success icon */}
          <div className="w-20 h-20 bg-green-500 flex items-center justify-center mx-auto mb-8 rounded-full animate-[scale-in_0.4s_ease-out]">
          ...
          {orderRef && (
            <p className="text-xs font-mono text-oryn-black/30 mb-2 mt-4 py-2 px-4 bg-oryn-grey-light inline-block">
              {t.checkoutPage.orderRef}: {orderRef}
            </p>
          )}

          {/* Order summary */}
          <div className="flex items-center justify-center gap-6 text-sm text-oryn-black/50 mb-6 mt-2">
            {orderItemsCount > 0 && (
              <span>
                {orderItemsCount} {orderItemsCount === 1 ? t.cart.itemLabel : t.cart.itemsLabel}
              </span>
            )}
            {orderTotal > 0 && <span className="font-bold text-oryn-black">{formatPrice(orderTotal)}</span>}
          </div>
          ...
          {/* What happens next */}
          <div className="bg-oryn-grey-light/50 border border-oryn-grey/20 p-5 mb-6 text-left">
            <p className="text-[10px] font-mono text-oryn-black/40 tracking-[0.15em] mb-3">
              {t.checkoutPage.whatHappensNext}
            </p>
            <div className="space-y-3">
              {(t.checkoutPage.nextSteps || [
                ...
                <div key={i} className="flex items-start gap-3">
                  <div className="w-5 h-5 bg-oryn-orange/10 flex items-center justify-center shrink-0 mt-0.5">
                    <span className="text-[10px] font-bold text-oryn-orange">
                      {i + 1}
                    </span>
                  </div>
                  <p className="text-sm text-oryn-black/60 font-plex">{step}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            ...
            {user && (
              <Link
                href="/account"
                className="px-8 py-3 border border-oryn-grey/30 text-sm font-medium hover:border-oryn-orange transition-colors inline-block"
              >
                {t.checkoutPage.viewOrders || "VIEW ORDERS"}
              </Link>
            )}
          </div>
        </div>
    ...
    <div className="pt-20 pb-16 min-h-screen bg-white">
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1.5 font-medium transition-colors ${
                      isActive
                        ? "text-oryn-orange"
                        : isCompleted
                          ? "text-oryn-black cursor-pointer hover:text-oryn-orange"
                          : "text-oryn-black/30"
                    }`}
                  >
                    <div
                      className={`w-5 h-5 flex items-center justify-center text-[10px] font-bold ${
                        isCompleted
                          ...
                          : isActive
                            ...
                            : "bg-oryn-grey-light text-oryn-black/30"
                      }`}
                    >
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        <button
          ...
          className="lg:hidden w-full flex items-center justify-between p-4 bg-oryn-grey-light mb-6 border border-oryn-grey/20 active:bg-oryn-grey-light/80 min-h-[44px]"
        >
          <div className="flex items-center gap-2">
            ...
            <div className="border border-oryn-grey/20">
              ...
              <div
                className={`px-6 py-4 flex items-center justify-between cursor-pointer ${
                  activeStep === "contact_delivery"
                    ? "bg-white"
                    : "bg-oryn-grey-light/50"
                }`}
                onClick={() =>
                  completedSteps.has("contact_delivery")
                    ? editStep("contact_delivery")
                    : undefined
                }
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-medium text-sm">
                    {t.checkoutPage.contactShipping || "Contact & Delivery"}
                  </span>
                </div>
                {completedSteps.has("contact_delivery") &&
                  activeStep !== "contact_delivery" && (
                    <button
                      onClick={() => editStep("contact_delivery")}
                      className="text-xs text-oryn-orange font-medium hover:underline"
                    >
                ...
                activeStep !== "contact_delivery" && (
                  <div className="px-6 pb-4 text-sm text-oryn-black/60 space-y-0.5 bg-oryn-grey-light/50">
                    ...
                    <p>
                      ...
                      {apartment ? `, ${apartment}` : ""}, {city} {postalCode},{" "}
                      {getCountryName(country, locale)}
                    </p>
                    {phone && <p>{phone}</p>}
                    {!billingSameAsShipping && (
                      <p className="text-[10px] font-mono text-oryn-black/30 mt-1">
                        BILLING: {billingFirstName} {billingLastName},{" "}
                        {billingAddress}, {billingCity}{" "}
                        {billingPostalCode},{" "}
                        {getCountryName(billingCountry, locale)}
                      </p>
                    )}
                  </div>
                )}
              ...
              {activeStep === "contact_delivery" && (
                <div className="px-6 pb-6 space-y-4">
                  {/* ── Email ── */}
                  <div>
                    <label className="block text-[10px] font-mono text-oryn-black/40 tracking-wider mb-1.5 mt-2">
                      EMAIL *
                    </label>
                    <input
                      type="email"
                      ...
                      className={`${inputCls(!!errors.email)} ${
                        user?.email
                          ? "bg-oryn-grey-light text-oryn-black/60 cursor-not-allowed"
                          : ""
                      }`}
                      autoComplete="email"
                      autoFocus
                  ...
                  <div className="pt-2">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {savedAddresses.map((addr) => (
                            <button
                              ...
                              className="px-3 py-1.5 text-[10px] font-medium border border-oryn-orange/20 hover:border-oryn-orange hover:bg-oryn-orange hover:text-white transition-colors min-h-[44px] flex items-center"
                            >
                              {addr.label}
                              {addr.isDefault && " *"}
                            </button>
                    ...
                    <div className="mb-3">
                      <input
                        ...
                        className={`${INPUT_BASE} border-oryn-grey/30`}
                  ...
                  <div className="border-t border-oryn-grey/20 pt-4">
                    <label className="flex items-center gap-3 cursor-pointer min-h-[44px]">
                      <div
                        className={`w-5 h-5 border-2 flex items-center justify-center transition-colors ${
                          billingSameAsShipping
                            ? "bg-oryn-orange border-oryn-orange"
                            : "border-oryn-grey/40"
                        }`}
                      >
                        {billingSameAsShipping && (
                      ...
                      <span className="text-sm text-oryn-black/70">
                    ...
                    {/* Billing address fields (shown when unchecked) */}
                    {!billingSameAsShipping && (
                      <div className="mt-4 space-y-3 pl-0 border-l-2 border-oryn-orange/20 ml-2.5 pl-4">
                        <p className="text-[10px] font-mono text-oryn-black/40 tracking-wider mb-3">
                          BILLING ADDRESS
                        </p>
                        ...
                        <select
                          ...
                          className={`${INPUT_BASE} appearance-none ${
                            errors.billingCountry
                              ? "border-red-300"
                              : "border-oryn-grey/30"
                          } ${!billingCountry ? "text-oryn-black/40" : ""}`}
                        >
                          <option value="">
                            {t.checkoutPage.selectCountry}
                          </option>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          <div>
                            <input
                              ...
                              placeholder={
                                t.checkoutPage.firstNamePlaceholder
                              }
                              className={inputCls(
                          ...
                          <div>
                            <input
                              ...
                              placeholder={
                                t.checkoutPage.lastNamePlaceholder
                              }
                              className={inputCls(
                        ...
                        <input
                          ...
                          placeholder={t.checkoutPage.addressPlaceholder}
                          className={inputCls(!!errors.billingAddress)}
                        ...
                        <input
                          ...
                          placeholder={
                            t.checkoutPage.apartmentPlaceholder
                          }
                          className={`${INPUT_BASE} border-oryn-grey/30`}
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          ...
                          <div>
                            <input
                              ...
                              placeholder={
                                t.checkoutPage.postalCodePlaceholder
                              }
                              className={inputCls(
                  ...
                  <button
                    ...
                    className="w-full py-4 bg-oryn-orange text-white font-bold text-sm tracking-wide hover:bg-oryn-orange-dark transition-colors disabled:opacity-50 min-h-[52px]"
                  >
                    {isSubmitting ? (
                      <span className="flex items-center justify-center gap-2">
                        <Spinner size={16} /> {t.checkoutPage.processing}
                      </span>
                    ) : (
                      <span className="flex items-center justify-center gap-2">
            ...
            <div className="border border-t-0 border-oryn-grey/20">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-medium text-sm">
                    {t.checkoutPage.shippingMethod || "Shipping"} &{" "}
                    {t.checkoutPage.paymentTitle || "Payment"}
                  </span>
                </div>
              </div>
              ...
              {activeStep === "shipping_payment" && (
                <div className="px-6 pb-6">
                  {/* Address summary */}
                  <div className="p-3 bg-oryn-grey-light/70 border border-oryn-grey/20 mb-6 text-sm text-oryn-black/60">
                    <div className="flex justify-between items-start">
                      <div>
                        <p className="font-medium text-oryn-black">
                          {firstName} {lastName}
                        </p>
                        <p>
                          {address}
                        ...
                        <p className="text-oryn-black/40 text-xs mt-0.5">
                          {email}
                        </p>
                      </div>
                      <button
                        onClick={() => editStep("contact_delivery")}
                        className="text-xs text-oryn-orange hover:underline shrink-0"
                      >
                        {t.checkoutPage.change}
                      </button>
                    </div>
                  ...
                  <div className="mb-6">
                    <p className="text-[10px] font-mono text-oryn-black/40 tracking-wider mb-3">
                      {t.checkoutPage.shippingMethod || "SHIPPING METHOD"}
                    </p>

                    {shippingLoading ? (
                      <div className="flex items-center justify-center py-8 gap-3">
                        <Spinner size={20} />
                        <span className="text-sm text-oryn-black/50">
                          {t.checkoutPage.loadingShipping}
                        </span>
                      </div>
                    ) : (
                      <>
                        ...
                        {qualifiesForFreeShipping && (() => {
                          const selectedOpt = shippingOptions.find(
                            (o) => o.id === selectedShipping
                          );
                          const savingsAmount =
                            selectedOpt && selectedOpt.amount > 0
                              ? selectedOpt.amount
                              : 0;
                          return (
                        ...
                        <div className="space-y-3">
                          {shippingOptions
                            .filter(
                              (opt) =>
                                !opt.region_id ||
                                opt.region_id === cart?.region_id
                            )
                            .map((option) => {
                              const displayAmount =
                                qualifiesForFreeShipping ? 0 : option.amount;
                              return (
                                <label
                                  key={option.id}
                                  onClick={() =>
                                    handleShippingSelect(option.id)
                                  }
                                  className={`flex items-center justify-between p-4 border cursor-pointer transition-all min-h-[52px] ${
                                ...
                                >
                                  <div className="flex items-center gap-3">
                                    ...
                                    >
                                      {selectedShipping === option.id && (
                                        <div className="w-2 h-2 rounded-full bg-oryn-orange" />
                                      )}
                                    </div>
                                    <div>
                                      ...
                                      {(() => {
                                        ...
                                        return est ? (
                                          <p className="text-[11px] text-oryn-black/45 mt-0.5">
                                            {est}
                                          </p>
                                        ) : null;
                                      })()}
                                      ...
                                      <span className="text-oryn-orange">
                                        ...
                                        option.amount > 0 ? (
                                          <span className="flex items-center gap-1.5">
                                            <span className="line-through text-oryn-black/30 text-xs">
                    ...
                    {[
                      {
                        ...
                        label: t.checkoutPage.sslEncrypted,
                        sub: "256-bit",
                      },
                      {
                        icon: "M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z",
                        label: t.checkoutPage.purity99,
                        sub: t.checkoutPage.guaranteed,
                      },
                      {
                        icon: "M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z",
                        label: t.checkoutPage.gmpCertifiedBadge,
                        sub: "ISO 7",
                      },
                      {
                        icon: "M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15",
                        label: t.checkoutPage.guarantee30,
                        sub: t.checkoutPage.noQuestions,
                      },
                    ].map((badge) => (
                      <div
                        key={badge.label}
                        className="flex items-center gap-2.5 p-3 bg-green-50/80 border border-green-200/50"
                      >
                        <svg
                          width="16"
                          height="16"
                        ...
                        <div>
                          ...
                          <p className="text-[8px] text-green-600/70 font-plex">
                            {badge.sub}
                          </p>
                        </div>
                      </div>
                  ...
                  <div className="mb-4">
                    ...
                    {medusaConnected ? (
                      <StripeCheckout
                        ...
                        amount={
                          cart?.total != null ? cart.total : finalTotal
                        }
                        formatPrice={formatPrice}
                      ...
                      <div className="p-6 bg-yellow-50 border border-yellow-200 text-yellow-800">
                        <p className="font-mono font-bold text-sm mb-1">
                          {t.checkoutPage.backendRequired}
                        </p>
                        <p className="text-xs font-plex">
                          {t.checkoutPage.backendDescription}
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  <button
                    onClick={() => editStep("contact_delivery")}
                    className="w-full mt-2 py-2 text-xs text-oryn-black/40 hover:text-oryn-orange transition-colors min-h-[44px]"
                  >
      ...
      {/* ─── Mobile sticky CTA (visible on mobile only) ──────────── */}
      <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-oryn-grey/20 p-4 z-40 safe-area-bottom">
        {activeStep === "contact_delivery" && (
          <button
            onClick={handleContinueToPayment}
            disabled={isSubmitting}
        ...
        {activeStep === "shipping_payment" && (
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[10px] font-mono text-oryn-black/40 tracking-wider">
                {t.checkoutPage.total}
              </p>
              <p className="text-lg font-bold">{formatPrice(finalTotal)}</p>
            </div>
...
function OrderSummary({
  ...
  const shippingEstimate = (() => {
    if (activeStep !== "contact_delivery") return null;
    ...
    <div className="bg-oryn-grey-light/50 border border-oryn-grey/20 p-6">
        ...
        {items.map((item) => (
          <div key={item.product.id} className="flex items-center gap-3">
            <div className="w-14 h-14 bg-oryn-cream border border-oryn-grey/20 flex items-center justify-center shrink-0 relative">
              <Image
                src={
                  productImages.bySlug[item.product.slug] ||
                  productImages.categoryCard[item.product.category] ||
          ...
          <div>
            <div className="flex gap-2">
              <input
                ...
                className={`flex-1 px-3 py-2.5 bg-white border text-[16px] sm:text-sm focus:outline-none focus:border-oryn-orange transition-colors font-mono ${
              ...
              <button
                ...
                className="px-4 py-2.5 bg-oryn-grey-dark text-white text-xs font-bold tracking-wider hover:bg-oryn-black transition-colors disabled:opacity-40 min-h-[44px]"
              >
                {promoLoading ? "..." : t.checkoutPage.apply}
              </button>
            </div>
            {promoError && (
              <p className="text-[10px] text-red-500 mt-1">{promoError}</p>
            )}
          </div>
        )}
      </div>
      ...
      <div className="mb-4">
        <input
          ...
          className="w-full px-3 py-2.5 bg-white border border-oryn-grey/30 text-[16px] sm:text-sm focus:outline-none focus:border-oryn-orange transition-colors font-mono"
        />
      </div>
        ...
        <div className="border-t border-oryn-grey/20 pt-3 pb-1">
          <div className="flex items-center gap-2 mb-1.5">
            ...
            <span className="text-[10px] font-mono text-oryn-orange tracking-wider">
              {t.cart.freeShippingAway.replace(
                "{amount}",
                formatPrice(freeShippingThreshold - totalPrice)
              )}
            </span>
          </div>
      ...
      <div className="border-t border-oryn-grey/20 pt-4 space-y-2">
        ...
        <div className="flex justify-between text-sm">
          <span className="text-oryn-black/50">
                ...
                ? shippingEstimate.isFree
                  ? (
                    <span className="text-green-600 text-xs font-mono">
                      {t.checkoutPage.free.toUpperCase()}
                    </span>
                  )
                  : (
                    <span className="text-xs text-oryn-black/60">
                      {t.cart.estimatedShippingFrom || "Est."}{" "}
                      {formatPrice(shippingEstimate.amount)}
                    </span>
                  )
                : (
                  <span className="text-xs text-oryn-black/30">
                    {t.checkoutPage.selectCountryForShipping ||
                      "Select country for estimate"}
                  </span>
                )
              : shippingCost === 0
                ? (
        ...
        {totalPrice >= freeShippingThreshold &&
          activeStep !== "contact_delivery" && (
            <div className="flex items-center gap-1.5 text-[10px] text-green-600 font-mono">
              ...
              {t.checkoutPage.freeShippingLabel.replace(
                "{threshold}",
      ...
      <div className="mt-6 pt-4 border-t border-oryn-grey/20 space-y-2">
        {[
          {
            icon: "M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z",
            label: t.checkoutPage.securePayment,
          },
          {
            icon: "M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4",
            label: t.checkoutPage.discreetShipping,
          },
          {
            icon: "M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z",
            label: t.checkoutPage.coaIncluded,
          },
          {
            icon: "M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4",
            label: t.checkoutPage.gmpCertified,
          },
        ].map((trust) => (
          <div key={trust.label} className="flex items-center gap-2">
            ...
            <span className="text-[9px] font-mono text-oryn-black/40 tracking-[0.05em]">
              {trust.label}
            </span>
          </div>
        ))}
      ...
      <div className="mt-4 pt-4 border-t border-oryn-grey/20">
        <p className="text-[8px] font-mono text-oryn-black/25 tracking-wider mb-2">
          {t.cart.weAccept || "ACCEPTED PAYMENT METHODS"}
        </p>
        <div className="flex flex-wrap gap-1.5">
          {[
            ...
            <span
              key={m}
              className="text-[9px] font-mono text-oryn-black/40 px-2 py-1 border border-oryn-grey/20 bg-white"
            >
              {m}
            </span>
          ))}
