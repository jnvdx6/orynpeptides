...
export async function generateMetadata({
  ...
  params: Promise<{ locale: string; country: string; city: string; product: string }>;
}): Promise<Metadata> {
  const { locale, country: countrySlug, city: citySlug, product: productSlug } = await params;
...
export default async function EuropeanCityProductPage({
  params,
}: {
  params: Promise<{ locale: string; country: string; city: string; product: string }>;
}) {
  ...
  const faqs = [
    {
      ...
      answer: `ORYN delivers ${product.name} peptide pens directly to ${city.name}, ${country.name} with ${city.deliveryDays}-day tracked delivery. Order online and receive your pre-mixed, research-grade pen at your door.`,
    },
    {
      question: `How much does ${product.name} cost?`,
      answer: `ORYN ${product.name} is priced at ${currency}${product.price} per pen, containing a 30-day supply of ${product.dosage} in a pre-mixed, ready-to-use pen system. Free delivery on qualifying orders.`,
    },
    {
      question: `How fast is delivery to ${city.name}?`,
      answer: `ORYN ships same-day for orders placed before 3 PM with ${city.deliveryDays}-day delivery to ${city.name}. All orders ship in discreet, temperature-controlled packaging.`,
    },
    {
      ...
      <MultiJsonLd
        items={[
          ...
          breadcrumbSchema([
            ...
            { name: product.name, url: `/${locale}/peptides/europe/${country.slug}/${city.slug}/${product.slug}` },
          ]),
        ]}
      />
      ...
      <div className="pt-[calc(1rem+4px)]">
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <nav className="flex items-center gap-2 text-[10px] font-mono text-oryn-black/30 tracking-[0.1em] flex-wrap">
            ...
            <span className="text-oryn-orange">/</span>
            <Link href={`/${locale}/peptides/europe/${country.slug}/${city.slug}`} className="hover:text-oryn-orange transition-colors">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
            <div className="bg-oryn-cream overflow-hidden relative flex items-center justify-center p-16 min-h-[400px]">
              <Image
                ...
                alt={`Buy ORYN ${product.name} in ${city.name}, ${country.name}`}
                width={350}
                height={350}
            ...
            <div>
              <div className="inline-flex items-center gap-3 mb-3">
                ...
                <span className="text-[10px] font-mono text-oryn-orange tracking-[0.2em]">
                  {product.categoryLabel.toUpperCase()} &mdash; {city.name.toUpperCase()}, {country.code}
                </span>
              </div>
              ...
              <p className="text-sm text-oryn-black/40 font-plex mb-4">
                {product.subtitle} &middot; {city.deliveryDays}-Day Delivery to {city.name}
              </p>
        ...
        <section className="max-w-4xl mx-auto px-6 py-12">
          <h2 className="text-xl font-bold mb-6">
            {product.name} in {city.name} &mdash; FAQ
          </h2>
          ...
          <section className="max-w-7xl mx-auto px-6 py-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {related.map((p) => (
                <Link
                  key={p.id}
                  href={`/${locale}/peptides/europe/${country.slug}/${city.slug}/${p.slug}`}
        ...
        {/* Other Products in City */}
        <section className="bg-oryn-cream py-16">
          <div className="max-w-7xl mx-auto px-6">
            <h2 className="text-xl font-bold mb-6">
              More Peptides Available in {city.name}
            </h2>
        ...
        <section className="bg-oryn-black text-white py-12">
          <div className="max-w-4xl mx-auto px-6 text-center">
            <h2 className="text-2xl font-bold mb-4">
              Order {product.name} in {city.name}
            </h2>
            <p className="text-sm text-white/50 font-plex mb-6">
              {city.deliveryDays}-day tracked delivery. Pre-mixed &amp; ready to use. &gt;99% purity guaranteed.
            </p>
            <Link
              href={`/${locale}/products/${product.slug}`}
