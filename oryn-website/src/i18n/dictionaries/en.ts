import type { Dictionary } from "../types";

const en: Dictionary = {
  meta: {
    title: "Buy Peptides UK | Peptide Pen Systems from £99 | ORYN Peptide Labs",
    titleTemplate: "%s | ORYN Peptide Labs",
    description:
      "UK's leading peptide pen supplier. Buy BPC-157, NAD+, Tirzepatide, GHK-Cu & more in precision-dosed pen systems. >99% purity, GMP certified. Next-day UK delivery from £99. Free shipping over £150.",
    ogDescription:
      "Buy research-grade peptide pens in the UK. 10 peptides from £99. Pre-mixed reusable pen systems. GMP certified, >99% purity. Next-day UK delivery. Free shipping across Europe.",
  },

  header: {
    announcementText: "FREE SHIPPING ON ALL ORDERS OVER {threshold}",
    shopNow: "SHOP NOW",
    myAccount: "My Account",
    signIn: "Sign In",
    searchPlaceholder: "Search peptides...",
    noResults: "No results for",
    typeToSearch: "Type to search...",
    viewAll: "VIEW ALL PRODUCTS",
  },

  nav: {
    products: "Products",
    science: "Science",
    about: "About",
    contact: "Contact",
    compare: "Compare",
    learn: "Learn",
  },

  hero: {
    tagline: "PRECISION PEPTIDE PEN SYSTEMS",
    brandDescriptor: "Research-Grade Peptide Pen Systems",
    subtitle: "Research-Grade Peptides. Pre-Dosed. Delivered Tomorrow.",
    description:
      "Pharmaceutical-grade peptides in reusable pen systems with replaceable cartridges. Click-and-go convenience, precise dosing, 30-day supply. GMP certified. Manufactured in South Korea. Purity >99%.",
    explorePeptides: "EXPLORE PEPTIDES",
    shopFrom: "SHOP FROM",
    offerPill: "FIRST ORDER: 10% OFF — Use code WELCOME10",
    urgencyLine: "Free next-day UK delivery on orders over £150",
    bestsellerBadge: "BESTSELLER",
    bestsellerLine: "BPC-157 — Our #1 Research Peptide",
    ourScience: "OUR SCIENCE",
    purityLabel: "PURITY",
    dosingLabel: "DOSING",
    dosingValue: "30 Days",
    trustFreeShipping: "FREE SHIPPING",
    trustSecure: "SECURE PAYMENT",
    trustCOA: "COA INCLUDED",
    socialProofCount: "2,400+ orders",
    socialProofLabel: "shipped this quarter",
    stats: {
      productsValue: "10",
      productsLabel: "Products",
      purityValue: ">99%",
      purityLabel: "Purity",
      cleanroomValue: "ISO 7",
      cleanroomLabel: "Cleanroom",
      certifiedValue: "GMP",
      certifiedLabel: "Certified",
    },
  },

  categories: {
    label: "PRODUCT LINES",
    titleLine1: "Three Systems.",
    titleLine2: "One Standard.",
    description:
      "Every ORYN product is manufactured in ISO Class 7 cleanrooms with GMP certification and third-party validation.",
    from: "From",
    items: [
      {
        name: "Peptide Pen System",
        tagline: "30-Day Precision Dosing",
        description:
          "8 research peptides in multi-dose reusable pens. Pharma-grade formulations with digital dose display.",
      },
      {
        name: "MediT Pen",
        tagline: "Once-Weekly Injection",
        description:
          "Prefilled Tirzepatide 40mg pen. Dual GIP/GLP-1 action for metabolic research.",
      },
      {
        name: "NovaDose System",
        tagline: "Daily NAD+ Microdosing",
        description:
          "Advanced cartridge-based NAD+ delivery. Pharmaceutical-grade with near 100% bioavailability.",
      },
    ],
  },

  showcase: {
    label: "FEATURED",
    titleLine1: "Research-Grade",
    titleLine2: "Peptides",
    subtitle: "Pharmaceutical-grade compounds from \u20ac99. Reusable pen systems with 30-day precision dosing.",
    viewAll: "VIEW ALL PRODUCTS",
    trustShipping: "FREE SHIPPING",
    trustPurity: "PURITY >99%",
    trustCOA: "COA INCLUDED",
    trustReturn: "BATCH TRACEABLE",
  },

  science: {
    label: "OUR PROCESS",
    titleLine1: "Engineered",
    titleLine2: "Molecular Precision",
    description:
      "From raw material sourcing to final sterilization, every step in the ORYN manufacturing process is governed by pharmaceutical-grade protocols and validated by independent laboratories.",
    specs: {
      manufacturingLabel: "Manufacturing",
      manufacturingValue: "Korea",
      capacityLabel: "Capacity",
      capacityValue: "100K/mo",
      batchSizeLabel: "Batch Size",
      batchSizeValue: "20,000 ea",
      leadTimeLabel: "Lead Time",
      leadTimeValue: "45-60 days",
    },
    steps: [
      {
        title: "Synthesis",
        description:
          "Solid-phase peptide synthesis with proprietary purification protocols achieving >99% purity across all formulations.",
      },
      {
        title: "Formulation",
        description:
          "Pharma-grade aqueous formulations with PEG viscosity agents. pH-optimized (6.8-7.4) for stability and bioavailability.",
      },
      {
        title: "Sterilization",
        description:
          "Full aseptic chamber manufacturing with 0.22um filtration and gamma ray sterilization finish. ISO Class 7 cleanrooms.",
      },
      {
        title: "Validation",
        description:
          "Third-party QA/QC by POSTECH, UNIST, and SGS. Every batch includes Certificate of Analysis with full analytical data.",
      },
    ],
  },

  howItWorks: {
    label: "HOW IT WORKS",
    titleLine1: "Simple.",
    titleLine2: "Effective.",
    description: "From choosing your peptide to seeing results — our pen delivery system makes research effortless.",
    cta: "EXPLORE PEPTIDES",
    steps: [
      {
        title: "Choose Your Peptide",
        description: "Browse our range of >99% purity peptides. Each pen comes pre-mixed and ready to use.",
      },
      {
        title: "Order Securely",
        description: "Fast checkout with Stripe. Free next-day UK delivery on orders over £150.",
      },
      {
        title: "Receive Discreetly",
        description: "Plain packaging, temperature-controlled shipping. COA included with every order.",
      },
      {
        title: "Start Your Research",
        description: "Adjustable dosing via precision pen. No mixing, no vials, no waste.",
      },
    ],
  },

  quality: {
    label: "QUALITY ASSURANCE",
    titleLine1: "Pharmaceutical",
    titleLine2: "Standards",
    pillars: [
      {
        title: "ISO Class 7 Cleanroom",
        description:
          "Particle-free environments meeting pharmaceutical manufacturing standards.",
      },
      {
        title: "GMP Certified",
        description:
          "Consistent quality, proper documentation, and traceable production.",
      },
      {
        title: ">99% Purity",
        description:
          "HPLC analysis and third-party validation by POSTECH, UNIST, and SGS.",
      },
      {
        title: "Batch Traceability",
        description:
          "Full DMF documentation, stability data, and COA for every batch.",
      },
      {
        title: "Gamma Ray Sterilization",
        description:
          "Complete sterility via gamma irradiation after aseptic manufacturing.",
      },
      {
        title: "European Standards",
        description:
          "Full compliance under European pharmaceutical regulatory frameworks.",
      },
    ],
  },

  cta: {
    label: "START YOUR RESEARCH",
    titleLine1: "Ready to advance your",
    titleLine2: "peptide research?",
    description:
      "Explore our complete catalog of research-grade peptides. Every product ships with Certificate of Analysis and full batch documentation.",
    browseProducts: "BROWSE ALL PRODUCTS",
    contactTeam: "CONTACT OUR TEAM",
    valueProp1: "FREE SHIPPING EU/UK",
    valueProp2: "COA WITH EVERY ORDER",
    valueProp3: "GMP CERTIFIED",
    badges: [
      { value: "ISO 7", label: "CLEANROOM" },
      { value: "GMP", label: "CERTIFIED" },
      { value: ">99%", label: "PURITY" },
      { value: "SGS", label: "VALIDATED" },
    ],
  },

  footer: {
    description:
      "Precision Peptide Science. Biotech laboratory delivering research-grade peptide solutions engineered for molecular precision.",
    researchOnly: "FOR RESEARCH PURPOSES ONLY",
    newsletterTitle: "Stay Updated",
    newsletterDescription: "Get the latest on new peptides, research updates, and exclusive offers.",
    newsletterPlaceholder: "your@email.com",
    newsletterButton: "SUBSCRIBE",
    newsletterSuccess: "Thank you! You're now subscribed.",
    paymentMethods: "WE ACCEPT",
    sections: {
      products: "Products",
      company: "Company",
      legal: "Legal",
    },
    productLinks: {
      all: "All Products",
      pens: "Peptide Pens",
      medit: "MediT Pen",
      novadose: "NovaDose",
    },
    companyLinks: {
      about: "About ORYN",
      science: "Science",
      contact: "Contact",
      quality: "Quality",
      whyOryn: "Why ORYN",
      researchHub: "Research Hub",
      calculator: "Peptide Calculator",
    },
    trustBadges: {
      ssl: "SSL Encrypted",
      secure: "Secure Checkout",
      coa: "COA Included",
      guarantee: "Purity Guaranteed",
    },
    legalLinks: {
      terms: "Terms of Service",
      privacy: "Privacy Policy",
      disclaimer: "Research Disclaimer",
      cookieSettings: "Cookie Settings",
    },
    seoSections: {
      researchAreas: "Research Areas",
      learn: "Learn",
      ukDelivery: "UK Delivery",
      ukRegions: "UK Regions",
      ukCounties: "UK Counties",
      peptideEncyclopedia: "Peptide Encyclopedia",
      resources: "Resources",
      londonDelivery: "London Delivery",
      europeanDelivery: "European Delivery",
      topEuCities: "Top EU Cities",
    },
    certifications: [
      { label: "ISO CLASS 7", detail: "Cleanroom" },
      { label: "GMP", detail: "Certified" },
      { label: ">99%", detail: "Purity" },
      { label: "KOREA", detail: "Manufactured" },
    ],
    copyright: "ORYN PEPTIDE LABS — ALL RIGHTS RESERVED",
  },

  productCard: {
    purity: "PURITY >99%",
    pharmaGrade: "PHARMA GRADE",
    details: "DETAILS",
    addToCart: "ADD TO CART",
  },

  cart: {
    title: "YOUR ORDER",
    empty: "Your cart is empty",
    emptySubtext: "Start adding research peptides to your cart.",
    browseProducts: "BROWSE PRODUCTS",
    continueShopping: "Continue Shopping",
    subtotal: "Subtotal",
    checkout: "SECURE CHECKOUT",
    researchOnly: "For research purposes only",
    itemLabel: "item",
    itemsLabel: "items",
    freeShippingAway: "Add {amount} more for FREE shipping",
    freeShippingUnlocked: "You've unlocked FREE shipping!",
    youMightLike: "YOU MIGHT ALSO LIKE",
    trustSecure: "SECURE PAYMENT",
    trustDiscreet: "DISCREET SHIPPING",
    trustCOA: "COA INCLUDED",
    orderSummary: "ORDER SUMMARY",
    shipping: "Shipping",
    free: "FREE",
    atCheckout: "AT CHECKOUT",
    estimatedShippingFrom: "From",
    weAccept: "ACCEPTED PAYMENT METHODS",
    total: "Total",
    volumeDiscount: "Volume discount",
    saveForLater: "Save for later",
    saved: "SAVED",
    save: "SAVE",
    havePromoCode: "Have a promo code?",
    cancel: "Cancel",
  },

  productsPage: {
    label: "CATALOG",
    title: "Research Peptides",
    description:
      "Explore our complete range of precision-engineered peptide solutions. Every product manufactured to pharmaceutical standards with >99% purity.",
    all: "ALL",
    results: "products",
    searchPlaceholder: "Search peptides...",
    sortBy: "SORT BY",
    sortPriceAsc: "PRICE: LOW TO HIGH",
    sortPriceDesc: "PRICE: HIGH TO LOW",
    sortName: "NAME: A-Z",
    noResults: "No products found for",
    noCategory: "No products in this category",
    clearFilters: "Clear filters",
    disclaimer:
      "ALL ORYN PRODUCTS ARE INTENDED FOR RESEARCH AND LABORATORY USE ONLY. NOT FOR HUMAN CONSUMPTION.",
  },

  productDetail: {
    notFound: "Product Not Found",
    backToProducts: "Back to Products",
    home: "HOME",
    products: "PRODUCTS",
    perUnit: "per unit",
    addToCart: "ADD TO CART",
    buyNow: "BUY NOW",
    orPayInstantly: "OR PAY INSTANTLY",
    adding: "ADDING...",
    keyBenefits: "KEY BENEFITS",
    specifications: "SPECIFICATIONS",
    researchOnlyTitle: "Research Use Only",
    researchOnlyDescription:
      "This product is intended for laboratory and research purposes only. Not for human consumption.",
    relatedProducts: "Related Products",
    trustShipping: "FREE SHIPPING",
    trustPurity: "PURITY >99%",
    trustCOA: "COA INCLUDED",
    secureCheckout: "SECURE CHECKOUT",
    discreetShipping: "DISCREET SHIPPING",
    inStock: "IN STOCK",
    readyToShip: "Ready to ship",
    reviews: "reviews",
    scienceTab: "Science",
    readyToExperience: "Ready to experience",
    premiumPenDescription: "Premium pen delivery system with fully adjustable dosing. GMP manufactured, >99% purity guaranteed.",
    dosingProtocol: "DOSING PROTOCOL",
    howToUse: "How to Use Your ORYN Pen",
    quickReference: "QUICK REFERENCE",
    recommendedDose: "Recommended Dose",
    frequency: "Frequency",
    duration: "Duration",
    proTips: "PRO TIPS",
    stepByStep: "STEP-BY-STEP INSTRUCTIONS",
    importantLabel: "IMPORTANT",
    safetyNote: "Always use a new sterile needle for each administration. Dispose of used needles in an appropriate sharps container. Store pen refrigerated at 2-8°C.",
    scienceLabel: "SCIENCE",
    scienceBehind: "The Science Behind",
    compoundProfile: "COMPOUND PROFILE",
    classificationLabel: "CLASSIFICATION",
    molecularFormula: "MOLECULAR FORMULA",
    molecularWeightLabel: "MOLECULAR WEIGHT",
    halfLifeLabel: "HALF-LIFE",
    sequenceLabel: "SEQUENCE",
    researchAreasLabel: "RESEARCH AREAS",
    mechanismOfAction: "MECHANISM OF ACTION",
    keyResearchFindings: "KEY RESEARCH FINDINGS",
    trustedByResearchers: "TRUSTED BY RESEARCHERS",
    whatLabsSay: "What Labs Say About",
    fromVerifiedReviews: "from {count} verified reviews",
    verifiedLabel: "VERIFIED",
    faqLabel: "FAQ",
    frequentlyAsked: "Frequently Asked About",
    buyInYourCity: "BUY {product} IN YOUR CITY",
    relatedResearch: "RELATED RESEARCH",
    formulaLabel: "FORMULA",
    weightLabel: "WEIGHT",
    add: "ADD",
    closeCart: "Close cart",
    decreaseQuantity: "Decrease quantity",
    increaseQuantity: "Increase quantity",
    removeItem: "Remove item",
    toggleWishlist: "Toggle wishlist",
    closeSearch: "Close search",
    closePopup: "Close",
    purityBadge: ">99% PURITY",
    gmpBadge: "GMP CERTIFIED",
    freeShippingBadge: "FREE SHIPPING 150+",
    whyChoosePen: "WHY CHOOSE THE PEN",
    traditionalVials: "TRADITIONAL VIALS",
    orynPenSystem: "ORYN PEN SYSTEM",
    vialProblems: [
      { label: "Reconstitution required", detail: "Bacteriostatic water, syringe, precise mixing" },
      { label: "Dosing variability", detail: "Syringe markings can vary 10-20%" },
      { label: "Contamination risk", detail: "Rubber stopper punctured multiple times" },
      { label: "Rapid degradation", detail: "Potency drops after reconstitution" },
      { label: "Complex setup", detail: "5-10 minutes per preparation" },
      { label: "Travel impractical", detail: "Vials, syringes, water, needles..." },
    ],
    comparisonBar: [
      { vial: "5-10 min", pen: "30 sec", label: "Prep Time" },
      { vial: "~80%", pen: ">99%", label: "Dose Accuracy" },
      { vial: "Decreasing", pen: "Maintained", label: "Sterility" },
      { vial: "7-14 days", pen: "30 days", label: "Stability" },
    ],
    compareWith: "COMPARE WITH SIMILAR PRODUCTS",
    sideBy: "Compare Products",
    vsLabel: "HIDE",
    keyDifferences: "KEY BENEFITS",
    bestFor: "benefits",
    viewProduct: "ADD TO CART",
    comparisonDisclaimer: "PRICE",
  },

  aboutPage: {
    tagline: "ABOUT ORYN",
    heroTitle1: "The Science Behind",
    heroTitle2: "the Precision",
    heroDescription:
      "ORYN is a biotech laboratory dedicated to advancing peptide research through pharmaceutical-grade manufacturing in South Korea, innovative delivery systems, and uncompromising quality standards.",
    brandStatement:
      "We believe peptide research deserves the same manufacturing rigor as",
    brandHighlight: "pharmaceutical production.",
    brandParagraphs: [
      "ORYN was born from a frustration with the peptide research market. Too many compounds of uncertain purity, inconsistent dosing, and opaque supply chains. We set out to change that.",
      "Our manufacturing partners operate ISO Class 7 cleanrooms with GMP certification. Every formulation undergoes HPLC analysis and independent third-party validation. We provide full batch documentation because transparency is not optional \u2014 it\u2019s foundational.",
      "The result: research-grade peptides that scientists and laboratories can trust, delivered in precision-engineered systems designed for consistent, reliable results.",
    ],
    valuesLabel: "OUR VALUES",
    valuesTitle: "What Drives",
    values: [
      {
        title: "Precision",
        description:
          "Every measurement matters. From synthesis to final fill, we control variables at the molecular level.",
      },
      {
        title: "Purity",
        description:
          "99% is our minimum. Third-party validated by POSTECH, UNIST, and SGS for absolute confidence.",
      },
      {
        title: "Innovation",
        description:
          "Three unique delivery platforms engineered for optimal stability, dosing accuracy, and user experience.",
      },
      {
        title: "Transparency",
        description:
          "Full batch documentation, COA for every product, and complete supply chain traceability.",
      },
    ],
    journeyLabel: "OUR JOURNEY",
    journeyTitle: "Building",
    timeline: [
      {
        year: "Foundation",
        title: "Precision from Day One",
        description:
          "ORYN was founded with a singular vision: to bring pharmaceutical-grade peptide research to European laboratories with uncompromising quality standards.",
      },
      {
        year: "Manufacturing",
        title: "Korean Biotech Partnership",
        description:
          "Strategic partnership with ISO Class 7 certified manufacturing facilities in Eumseong and Osan, Korea. 100,000 units per month capacity.",
      },
      {
        year: "Innovation",
        title: "Advanced Delivery Systems",
        description:
          "Development of three proprietary delivery platforms: multi-dose Peptide Pen, prefilled MediT Pen, and the NovaDose cartridge system.",
      },
      {
        year: "Today",
        title: "10 Research Peptides",
        description:
          "Complete catalog of research-grade peptides covering regeneration, metabolic, longevity, and growth hormone research with global distribution.",
      },
    ],
    manufacturingLabel: "MANUFACTURING",
    manufacturingTitle: "Production Specifications",
    manufacturingSpecs: [
      { value: "ISO 7", label: "Cleanroom Class", sub: "Certified" },
      { value: "GMP", label: "Manufacturing", sub: "Certified" },
      { value: "100K", label: "Monthly Capacity", sub: "Units/Month" },
      { value: "24mo", label: "Shelf Life", sub: "Stability Tested" },
      { value: "0.22um", label: "Sterilization", sub: "Filtration" },
      { value: "SGS", label: "Validation", sub: "3rd Party" },
      { value: "6.8-7.4", label: "pH Range", sub: "Optimized" },
      { value: "PEG", label: "Viscosity", sub: "Agent" },
    ],
  },

  sciencePage: {
    tagline: "SCIENCE",
    heroTitle1: "Peptide",
    heroTitle2: "Research Library",
    heroDescription:
      "Understanding the molecular mechanisms behind every ORYN product. This library provides a scientific overview of our research peptide catalog and their areas of investigation.",
    processLabel: "PROCESS",
    processTitle1: "From Synthesis to",
    processTitle2: "Delivery",
    processDescription:
      "A six-stage pharmaceutical manufacturing pipeline ensuring consistency, purity, and stability in every product.",
    processSteps: [
      { step: "01", label: "Raw Material", time: "2 Weeks" },
      { step: "02", label: "Synthesis", time: "8 Days" },
      { step: "03", label: "Purification", time: "HPLC" },
      { step: "04", label: "Formulation", time: "pH 6.8-7.4" },
      { step: "05", label: "Fill & Seal", time: "2 Weeks" },
      { step: "06", label: "Sterilize & QC", time: "1 Week" },
    ],
    libraryLabel: "LIBRARY",
    libraryTitle1: "Peptide Compound",
    libraryTitle2: "Profiles",
    classification: "CLASSIFICATION",
    mechanism: "MECHANISM",
    researchAreas: "RESEARCH AREAS",
    specsLabel: "SPECIFICATIONS",
    specsTitle: "Universal Quality Parameters",
    specsItems: [
      { value: ">99%", label: "Purity (HPLC)" },
      { value: "6.8-7.4", label: "pH Range" },
      { value: "0.22um", label: "Sterile Filtration" },
      { value: "3 mL", label: "Fill Volume" },
      { value: "PEG", label: "Viscosity Agent" },
      { value: "30 Days", label: "Dosing Period" },
      { value: "24 mo", label: "Shelf Life" },
      { value: "2-8\u00b0C", label: "Storage Temp" },
    ],
  },

  contactPage: {
    tagline: "CONTACT",
    heroTitle: "Get in Touch",
    heroDescription:
      "For wholesale inquiries, research partnerships, or product questions. Our team responds within 24 hours.",
    companyName: "ORYN Peptide Labs",
    emailLabel: "EMAIL",
    wholesaleLabel: "WHOLESALE",
    locationLabel: "LOCATION",
    locationLine1: "European Operations",
    locationLine2: "Research & Distribution",
    infoBoxes: [
      {
        title: "Wholesale Orders",
        description:
          "MOQ: 1,000 per SKU. Full OEM branding available. Payment: 50% upfront / 50% before shipping. DDP shipping support available.",
      },
      {
        title: "Research Partnerships",
        description:
          "We support academic and institutional research programs with custom formulations, bulk pricing, and priority manufacturing.",
      },
      {
        title: "Documentation",
        description:
          "COA, stability data, DMF documentation, and marketing materials available for all ORYN products upon request.",
      },
    ],
    formTitle: "Send us a message",
    formDescription:
      "Fill out the form and our team will get back to you promptly.",
    firstName: "FIRST NAME",
    lastName: "LAST NAME",
    email: "EMAIL",
    organization: "ORGANIZATION",
    inquiryType: "INQUIRY TYPE",
    inquiryOptions: [
      "Product Inquiry",
      "Wholesale Order",
      "Research Partnership",
      "Custom Formulation",
      "Documentation Request",
      "Other",
    ],
    message: "MESSAGE",
    messagePlaceholder: "Tell us about your research needs...",
    sendMessage: "SEND MESSAGE",
    messageSent: "Message Sent",
    messageSentDescription: "We'll respond within 24 hours.",
  },

  checkoutPage: {
    emptyCart: "Your cart is empty",
    emptyCartDescription: "Add products before proceeding to checkout.",
    browseProducts: "Browse Products",
    steps: ["Information", "Shipping", "Payment"],
    shippingTitle: "Shipping Information",
    firstName: "FIRST NAME",
    lastName: "LAST NAME",
    email: "EMAIL",
    phone: "PHONE",
    address: "ADDRESS",
    city: "City",
    postalCode: "Postal Code",
    country: "Country",
    referralCode: "REFERRAL CODE",
    referralCodePlaceholder: "Enter referral code",
    referralCodeHint: "Have a referral code? Enter it for tracking.",
    continueToPayment: "CONTINUE TO PAYMENT",
    paymentTitle: "Payment",
    cardPayment: "Card Payment",
    cryptoPayment: "Crypto Payment",
    cardNumber: "CARD NUMBER",
    expiry: "EXPIRY",
    cvc: "CVC",
    selectCrypto: "SELECT CRYPTOCURRENCY",
    amountToPay: "AMOUNT TO PAY",
    discountApplied: "(5% discount applied)",
    sendToAddress: "SEND TO ADDRESS",
    copy: "COPY",
    copied: "COPIED",
    timeRemaining: "TIME REMAINING",
    sendBeforeExpiry: "Send payment before timer expires",
    sentPayment: "I'VE SENT THE PAYMENT",
    txHash: "TRANSACTION HASH",
    txHashPlaceholder: "Enter your transaction hash",
    sessionExpired: "PAYMENT SESSION EXPIRED",
    restartSession: "RESTART SESSION",
    back: "Back",
    placeOrder: "PLACE ORDER",
    processing: "PROCESSING...",
    orderSummary: "ORDER SUMMARY",
    qty: "Qty",
    subtotal: "Subtotal",
    cryptoDiscount: "Crypto Discount (5%)",
    shipping: "Shipping",
    shippingNote: "CALCULATED AT NEXT STEP",
    referral: "Referral",
    total: "Total",
    researchOnly: "For research purposes only",
    orderConfirmed: "Order Confirmed",
    orderConfirmedDescription:
      "Thank you for your order. A confirmation email has been sent with your order details and tracking information.",
    paymentVerifying: "PAYMENT BEING VERIFIED",
    paymentVerifyingDescription:
      "Your crypto payment is being confirmed on the blockchain. This may take up to 30 minutes.",
    paymentConfirmed: "PAYMENT CONFIRMED",
    referralApplied: "REFERRAL CODE APPLIED",
    continueShopping: "Continue Shopping",
    secureCheckout: "SECURE CHECKOUT",
    orderRef: "ORDER REF",
    contactShipping: "Contact & shipping information",
    change: "Change",
    emailPlaceholder: "your@email.com",
    shippingAddress: "SHIPPING ADDRESS",
    useSavedAddress: "USE SAVED ADDRESS",
    selectCountry: "Select country...",
    firstNamePlaceholder: "First name",
    lastNamePlaceholder: "Last name",
    addressPlaceholder: "Address",
    apartmentPlaceholder: "Apartment, suite, etc. (optional)",
    cityPlaceholder: "City",
    postalCodePlaceholder: "Postal code",
    phonePlaceholder: "Phone (optional)",
    continueToShipping: "CONTINUE TO SHIPPING",
    shippingMethod: "Shipping method",
    loadingShipping: "Loading shipping options...",
    freeShippingApplied: "FREE SHIPPING APPLIED!",
    freeShippingLabel: "FREE SHIPPING (ORDER +£{threshold})",
    freeStandardShipping: "Free standard shipping",
    returnToInformation: "← Return to information",
    returnToShipping: "← Return to shipping",
    contact: "CONTACT",
    shipTo: "SHIP TO",
    method: "METHOD",
    free: "Free",
    required: "Required",
    validEmailRequired: "Valid email required",
    invalidCode: "Invalid code",
    failedToValidate: "Failed to validate",
    hideOrderSummary: "Hide order summary",
    showOrderSummary: "Show order summary",
    discount: "Discount",
    volumeDiscountLabel: "Volume discount",
    calculatedNextStep: "Calculated at next step",
    selectCountryForShipping: "Select country for estimate",
    securePayment: "SECURE PAYMENT",
    discreetShipping: "DISCREET SHIPPING",
    coaIncluded: "COA INCLUDED",
    gmpCertified: "GMP CERTIFIED",
    allSecure: "ALL TRANSACTIONS ARE SECURE AND ENCRYPTED",
    backendRequired: "BACKEND REQUIRED",
    backendDescription: "Medusa backend is required to process payments.",
    promoCodePlaceholder: "Discount code",
    apply: "APPLY",
    referralPlaceholder: "Referral code (optional)",
    sslEncrypted: "SSL Encrypted",
    purity99: ">99% Purity",
    guaranteed: "Guaranteed",
    gmpCertifiedBadge: "GMP Certified",
    guarantee30: "30-Day Guarantee",
    noQuestions: "No questions",
    emailHint: "No account needed. Your order will be linked to this email.",
    verifyingPayment: "Verifying payment...",
    paymentFailed: "Payment Failed",
    paymentFailedDescription: "Your payment could not be processed. Please try again or use a different payment method.",
    tryAgain: "Try Again",
    whatHappensNext: "WHAT HAPPENS NEXT",
    nextSteps: [
      "Order confirmation email sent to your inbox",
      "Your order is being prepared & quality checked",
      "Shipped with tracked delivery — usually next business day",
    ],
    viewOrders: "VIEW ORDERS",
    referralTitle: "Share ORYN & earn 10% commission",
    referralDescription: "Love our products? Share your referral link and earn 10% on every purchase.",
    getReferralLink: "GET YOUR REFERRAL LINK →",
  },

  products: {
    "bpc-157": {
      subtitle: "Regeneration Research Peptide",
      categoryLabel: "Peptide Pen",
      description:
        "Body Protection Compound-157 is a synthetic pentadecapeptide derived from a naturally occurring protein in gastric juice. ORYN BPC-157 is formulated at pharmaceutical grade in a precision-dosed pen system for consistent, reliable administration over 30 days.",
      benefits: [
        "Investigated for tissue regeneration and recovery pathways",
        "Studied for gastrointestinal mucosal integrity research",
        "Research focus on tendon and ligament repair mechanisms",
        "Investigated for anti-inflammatory pathway modulation",
        "Studied for neuroprotective mechanisms",
      ],
      badge: "Best Seller",
    },
    "tb-500": {
      subtitle: "Cell Migration Research Peptide",
      categoryLabel: "Peptide Pen",
      description:
        "Thymosin Beta-4 fragment, TB-500, is widely studied for its role in cell migration and tissue regeneration. ORYN TB-500 delivers a precise 15mg dose in our advanced pen system, engineered for optimal bioavailability and consistent results.",
      benefits: [
        "Investigated for wound closure research",
        "Studied for inflammatory pathway modulation",
        "Research focus on muscle tissue regeneration",
        "Investigated in cardiovascular tissue models",
        "Studied for flexibility and mobility research",
      ],
    },
    "cjc-1295": {
      subtitle: "GH Secretagogue Research Peptide",
      categoryLabel: "Peptide Pen",
      description:
        "CJC-1295 is a synthetic analogue of growth hormone-releasing hormone (GHRH). ORYN CJC-1295 is investigated for sustained GH pathway modulation through our precision dosing system, manufactured to the highest pharmaceutical standards.",
      benefits: [
        "Investigated for growth hormone release pathways",
        "Studied for body composition research",
        "Research focus on sleep architecture and GH pulsatility",
        "Investigated for recovery pathway modulation",
        "Studied for metabolic function research",
      ],
    },
    ipamorelin: {
      subtitle: "Selective GH Secretagogue",
      categoryLabel: "Peptide Pen",
      description:
        "Ipamorelin is a selective growth hormone secretagogue investigated for stimulating pituitary GH release. ORYN Ipamorelin is studied for targeted GH modulation without the broader hormonal disruption seen in less selective compounds.",
      benefits: [
        "Investigated for selective GH release pathways",
        "Studied for its selective receptor profile",
        "Research focus on bone density applications",
        "Investigated for lean tissue research",
        "Studied for sleep quality research",
      ],
    },
    "tirzepatide-pen": {
      subtitle: "Dual Incretin Research Peptide",
      categoryLabel: "Peptide Pen",
      description:
        "Tirzepatide is a dual GIP/GLP-1 receptor agonist representing the forefront of metabolic peptide science. ORYN Tirzepatide pen system delivers precision dosing for metabolic research applications.",
      benefits: [
        "Dual receptor agonism for metabolic research",
        "Studied for metabolic pathway modulation",
        "Investigated for glucose homeostasis research",
        "Research focus on appetite signalling pathways",
        "Extensively studied compound in clinical trials",
      ],
      badge: "Popular",
    },
    "ghk-cu": {
      subtitle: "Copper Peptide Research Compound",
      categoryLabel: "Peptide Pen",
      description:
        "GHK-Cu (Copper peptide) is a naturally occurring tripeptide with a high affinity for copper ions. ORYN GHK-CU is investigated for skin remodelling and tissue repair research with a potent 60mg formulation.",
      benefits: [
        "Investigated for collagen synthesis pathways",
        "Studied for skin elasticity and remodelling research",
        "Research focus on antioxidant mechanisms",
        "Investigated for wound closure research",
        "Studied for gene expression modulation",
      ],
    },
    glutathione: {
      subtitle: "Antioxidant Research Tripeptide",
      categoryLabel: "Peptide Pen",
      description:
        "Glutathione is a key endogenous antioxidant, widely studied for its role in cellular detoxification and redox balance. ORYN Glutathione delivers a research-grade 6g dose via our precision pen system for enhanced bioavailability.",
      benefits: [
        "Investigated for antioxidant defence research",
        "Studied for cellular detoxification pathways",
        "Research focus on immune system modulation",
        "Investigated for skin depigmentation mechanisms",
        "Studied for hepatic function research",
      ],
    },
    "nad-plus": {
      subtitle: "Cellular Coenzyme Research",
      categoryLabel: "Peptide Pen",
      description:
        "Nicotinamide Adenine Dinucleotide (NAD+) is a critical coenzyme present in every cell. ORYN NAD+ pen delivers a concentrated 500mg dose investigated for cellular energy metabolism, DNA repair pathways, and aging research.",
      benefits: [
        "Investigated for cellular energy metabolism",
        "Studied for DNA repair pathway research",
        "Research focus on healthy aging mechanisms",
        "Investigated for cognitive function research",
        "Studied for metabolic pathway efficiency",
      ],
      badge: "Premium",
    },
    "medit-tirzepatide": {
      subtitle: "Tirzepatide 40mg — Metabolic Research Pen",
      categoryLabel: "MediT Pen",
      description:
        "The ORYN MediT Pen is a prefilled, single-use injection pen containing 40mg Tirzepatide. Designed for once-weekly administration in metabolic research protocols, it features dual GIP/GLP-1 receptor agonism.",
      benefits: [
        "Dual receptor agonism (GIP + GLP-1) for metabolic research",
        "Once-weekly administration convenience",
        "Extensively studied in SURPASS/SURMOUNT clinical trials",
        "Investigated for appetite and satiety pathway modulation",
        "Studied for glucose homeostasis research",
        "Research focus on long-term metabolic outcomes",
        "Investigated in cardiovascular risk factor studies",
      ],
      badge: "New",
    },
    "novadose-nad": {
      subtitle: "Advanced NAD+ Delivery System",
      categoryLabel: "NovaDose System",
      description:
        "NovaDose delivers pharmaceutical-grade NAD+ through an innovative cartridge-based pen system. Designed for precise daily microdosing in research protocols, it is investigated for cellular energy, metabolic efficiency, and repair pathway research. Designed for sustained NAD+ research protocols.",
      benefits: [
        "Pharmaceutical-grade NAD+ from Korea",
        "Precise daily microdosing",
        "Near 100% bioavailability for research applications",
        "Investigated for cellular energy and metabolic research",
        "Studied for cognitive performance and recovery research",
        "Cost-effective alternative to IV-based research protocols",
        "GMP-certified manufacturing",
      ],
      badge: "Innovation",
    },
  },

  categoryNames: {
    "peptide-pen": "Peptide Pen System",
    "medit-pen": "MediT Pen",
    novadose: "NovaDose System",
  },

  categoryDescriptions: {
    "peptide-pen":
      "Multi-dose reusable pens with 30-day precision dosing. 8 research peptides available.",
    "medit-pen":
      "Prefilled single-use weekly injection pen. Tirzepatide 40mg for metabolic research.",
    novadose:
      "Advanced NAD+ cartridge delivery system for daily precision microdosing.",
  },

  researchCategories: {
    recovery: "Recovery Research",
    "weight-loss": "Metabolic Research",
    "anti-aging": "Longevity Research",
    "muscle-growth": "Muscle Tissue Research",
    "skin-rejuvenation": "Skin Biology Research",
    "sleep-quality": "Sleep & Recovery Research",
    "gut-health": "Gut Research",
    "joint-health": "Joint & Tendon Research",
    "hair-growth": "Hair Follicle Research",
    "immune-support": "Immune Research",
    "tendon-repair": "Tendon Research",
    "sports-recovery": "Sports Recovery Research",
    "post-surgery": "Post-Surgical Research",
    "cognitive-enhancement": "Cognitive Research",
    "energy-vitality": "Energy & Metabolism Research",
    "detox-cleanse": "Detoxification Research",
    "body-composition": "Body Composition Research",
    inflammation: "Inflammation Research",
    "hormonal-balance": "Hormonal Research",
    "longevity-biohacking": "Longevity & Biohacking Research",
  },

  breadcrumbs: {
    home: "Home",
    products: "Products",
    about: "About",
    science: "Science",
    contact: "Contact",
    learn: "Learn",
    quality: "Quality",
    faq: "FAQ",
    compare: "Compare",
    shipping: "Shipping",
    whyOryn: "Why ORYN",
    terms: "Terms",
    privacy: "Privacy",
    disclaimer: "Disclaimer",
    cart: "Cart",
    checkout: "Checkout",
    account: "Account",
    orders: "Orders",
    referrals: "Referrals",
    wishlist: "Wishlist",
    profile: "Profile",
    wholesale: "Wholesale",
    bundles: "Bundles",
    protocols: "Protocols",
    peptides: "Peptides",
    peptidePens: "Peptide Pens",
    europe: "Europe",
    glossary: "Glossary",
    london: "London",
    encyclopedia: "Encyclopedia",
    ukPeptideDelivery: "UK Peptide Delivery",
    tools: "Tools",
    peptideCalculator: "Peptide Calculator",
  },

  homeSeo: {
    deliveryTitle: "PEPTIDE DELIVERY ACROSS THE UK",
    researchAreaTitle: "PEPTIDES BY RESEARCH AREA",
    researchHubTitle: "RESEARCH HUB",
    peptidesFor: "Peptides for",
  },

  testimonials: {
    label: "TRUSTED BY RESEARCHERS",
    titleLine1: "What Our",
    titleLine2: "Clients Say",
    items: [
      {
        quote: "The purity and consistency of ORYN peptides is unmatched. The pen delivery system makes dosing incredibly precise and reliable for our lab protocols.",
        name: "Dr. M. Richter",
        role: "Research Director, Munich",
      },
      {
        quote: "We switched to ORYN six months ago. The COA documentation, batch traceability, and GMP certification give us complete confidence in every order.",
        name: "Dr. S. Lindberg",
        role: "Clinical Researcher, Stockholm",
      },
      {
        quote: "Outstanding product quality and fast EU shipping. The NovaDose system is a game-changer for our NAD+ research. Highly recommended for any serious lab.",
        name: "Prof. J. Torres",
        role: "Biotech Lab Director, Barcelona",
      },
    ],
    stats: [
      { value: "2,400+", label: "ORDERS SHIPPED" },
      { value: "98%", label: "REORDER RATE" },
      { value: "4.9/5", label: "SATISFACTION" },
      { value: "24h", label: "RESPONSE TIME" },
    ],
  },

  localeSwitcher: {
    label: "Market",
    uk: "UK (£)",
    eu: "EU (\u20ac)",
  },

  wishlistPage: {
    title: "Wishlist",
    breadcrumb: "WISHLIST",
    empty: "Your wishlist is empty",
    emptyTitle: "No saved items yet",
    emptyDescription: "Browse our products and click the heart icon to save items for later.",
    browseProducts: "BROWSE PRODUCTS",
    itemsSaved: "items saved",
    itemSaved: "item saved",
  },

  faq: {
    title: "Frequently Asked Questions",
    label: "FAQ",
  },

  contactFaq: {
    title: "BEFORE YOU CONTACT US",
    items: [
      { q: "How long does delivery take?", a: "UK orders: 2-4 business days. Europe: 3-7 business days. All orders ship in discreet, temperature-controlled packaging." },
      { q: "Can I track my order?", a: "Yes! Once dispatched, you'll receive a tracking number via email. You can also track orders in your account dashboard." },
      { q: "What is your return policy?", a: "We accept returns within 30 days of delivery. Unopened products in original packaging qualify for a full refund. For damaged or incorrect items, please report within 48 hours with photos — we will arrange a free replacement or refund. Return shipping is at your cost unless the issue was our error. Refunds are processed within 5-10 business days. Contact info@orynlabs.com to initiate a return." },
      { q: "Do you offer wholesale pricing?", a: "Yes, contact wholesale@orynlabs.com for bulk pricing. We offer tiered discounts for research institutions and resellers." },
    ],
  },

  homeFaq: {
    items: [
      { q: "What are ORYN Peptide Pens?", a: "ORYN Peptide Pens are pre-mixed, ready-to-use research peptide delivery systems. Each pen contains pharmaceutical-grade peptides at >99% purity, manufactured in our ISO 7 cleanroom facility. The pen format eliminates the need for reconstitution, ensuring consistent dosing for research applications." },
      { q: "Are peptides legal in the UK and Europe?", a: "Yes, research peptides are legal to purchase in the UK and across Europe for research purposes. ORYN Peptides are sold strictly for in-vitro research and laboratory use. They are not intended for human consumption." },
      { q: "How are your peptides manufactured?", a: "All ORYN peptides are synthesised in our GMP-certified, ISO 7 cleanroom laboratory in South Korea. Every batch undergoes rigorous HPLC and mass spectrometry testing to verify >99% purity. A Certificate of Analysis (COA) is included with every order." },
      { q: "How long does delivery take?", a: "UK orders typically arrive within 2-4 business days. European orders take 3-7 business days depending on destination. All orders are shipped in discreet, temperature-controlled packaging. Orders over \u20ac150 qualify for free shipping." },
      { q: "What is the ORYN Referral Programme?", a: "Our multi-level referral programme lets you earn 10% commission on purchases made by colleagues you refer. You also earn commissions up to 5 levels deep as your network grows. Sign up for a free account to get your unique referral code." },
      { q: "Do you offer bulk or wholesale pricing?", a: "Yes, we offer volume discounts starting from 3+ units. The more you order, the bigger the discount \u2014 up to 15% off for large research orders. Contact us for custom wholesale pricing on bulk orders." },
    ],
  },

  shippingFaq: {
    items: [
      { q: "How long does UK peptide delivery take?", a: "UK delivery times depend on your location. London and the South East receive orders in 1-2 business days, the Midlands, East, North England, and Wales in 2-3 business days, Scotland in 3-4 business days, and the Highlands and Islands in 4-5 business days. All orders placed before 2pm are dispatched the same day." },
      { q: "Is peptide shipping temperature-controlled?", a: "Yes. All ORYN peptide shipments are packed in insulated boxes with gel ice packs to maintain a temperature range of 2-8 degrees Celsius during transit. During summer months (June-September), we add additional cold chain protection including extra ice packs and insulated liners to ensure product integrity." },
      { q: "How do I qualify for free shipping?", a: "Free standard UK shipping is available on all orders over 150 pounds. This threshold applies to the order subtotal before any discount codes. International orders to Europe are subject to a flat shipping fee regardless of order value. Your cart will show a progress bar indicating how close you are to the free shipping threshold." },
      { q: "Do you ship peptides internationally to Europe?", a: "Yes, ORYN ships to most European Union countries. European delivery typically takes 3-7 business days depending on the destination country. All international shipments include temperature-controlled packaging and full tracking. Customs duties and import taxes are the responsibility of the customer." },
      { q: "How is my peptide order packaged?", a: "All ORYN orders are shipped in plain, unmarked boxes with no product descriptions or branding visible on the outside. Inside, peptide pens are secured in custom foam inserts within an insulated container with gel ice packs. A packing slip and Certificate of Analysis are included inside the box." },
      { q: "Can I track my peptide order?", a: "Yes. All orders receive a tracking number via email within 2 hours of dispatch. You can track your shipment in real time through our courier partner portal. For wholesale orders of 10+ units, we provide enhanced tracking with estimated delivery windows and delivery confirmation signatures." },
    ],
  },

  qualityFaq: {
    items: [
      { q: "What is a Certificate of Analysis (COA)?", a: "A Certificate of Analysis is a document issued by a qualified testing laboratory that confirms the identity, purity, and composition of a peptide product. Each ORYN COA includes HPLC purity data, mass spectrometry identity confirmation, endotoxin testing results, sterility verification, and batch-specific manufacturing details." },
      { q: "How do I read an HPLC purity result?", a: "HPLC (High-Performance Liquid Chromatography) results show a chromatogram with peaks representing different components. The main peak area percentage indicates purity \u2014 for example, 99.2% means 99.2% of the detected material is the target peptide. All ORYN peptides consistently achieve greater than 98% purity, with most batches exceeding 99%." },
      { q: "Who performs your third-party testing?", a: "ORYN peptides are tested by independent, ISO 17025-accredited analytical laboratories. These labs operate independently from our manufacturing facilities, ensuring unbiased verification of purity, identity, and sterility for every batch we produce." },
      { q: "How often are ORYN peptides tested?", a: "Every single batch of every ORYN product is tested before release. We operate a strict batch-release protocol: no product leaves our facility without a passing Certificate of Analysis from an independent laboratory. Testing includes HPLC purity analysis, mass spectrometry identity confirmation, endotoxin screening, and sterility verification." },
      { q: "What purity standards do ORYN peptides meet?", a: "All ORYN peptides are manufactured to exceed 98% purity, with most batches achieving greater than 99%. This is verified by independent HPLC testing and confirmed via mass spectrometry. Our manufacturing facilities are GMP-certified and operate under ISO 9001 quality management systems in ISO 7 cleanroom environments." },
      { q: "Can I request a COA for my specific batch?", a: "Yes. Every ORYN product ships with a batch number printed on the packaging. You can request the specific COA for your batch by contacting our support team at info@orynpeptides.com with your batch number. We maintain full traceability records for every batch manufactured." },
    ],
  },

  wholesaleFaq: {
    items: [
      { q: "What is the minimum order quantity for wholesale pricing?", a: "Our wholesale pricing tiers start at just 3 units. Orders of 3-5 units receive 5% off, 6-9 units receive 10% off, and 10 or more units receive 15% off retail pricing. For orders of 50+ units, we offer custom quotes tailored to your specific requirements." },
      { q: "How does wholesale pricing work at ORYN?", a: "ORYN wholesale pricing is based on volume discount tiers applied to our standard retail prices. Discounts are calculated automatically at checkout for qualifying orders. You can mix and match any products across our Peptide Pen, MediT Pen, and NovaDose ranges to reach your tier. For ongoing supply agreements, contact our wholesale team for tailored pricing." },
      { q: "How do I set up a wholesale account?", a: "Setting up a wholesale account is straightforward. Contact our team at info@orynpeptides.com or use the wholesale enquiry form on our contact page. We will verify your organisation details, assign a dedicated account manager, and activate volume pricing on your account within 1-2 business days." },
      { q: "What payment terms are available for wholesale customers?", a: "Standard wholesale orders are payable by bank transfer, credit card, or purchase order (for approved accounts). We offer NET-30 payment terms for established wholesale customers with a verified trading history. All first orders are prepaid. Contact our team to discuss payment arrangements for large-volume contracts." },
      { q: "What are delivery times for bulk peptide orders?", a: "Wholesale orders ship within 1-2 business days from our UK distribution centre. Standard UK delivery takes 2-4 business days. For large orders (50+ units), we may require up to 5 business days for preparation. All wholesale shipments include temperature-controlled packaging at no extra cost, with full tracking provided." },
    ],
  },

  account: {
    nav: {
      dashboard: "Dashboard",
      orders: "Orders",
      referrals: "Referrals",
      wishlist: "Wishlist",
      profile: "Profile",
      signOut: "Sign Out",
    },
    dashboard: {
      welcome: "Welcome back,",
      subtitle: "Manage your orders, track shipments, and update your account details.",
      yourOrders: "Your Orders",
      yourOrdersDesc: "View order history and track shipments",
      profileSettings: "Profile Settings",
      profileSettingsDesc: "Update your personal information",
      shopPeptides: "Shop Peptides",
      shopPeptidesDesc: "Browse our research-grade catalog",
      orynRewards: "ORYN Rewards",
      earnPoints: "Earn 1 point per \u20ac1 spent",
      points: "POINTS",
      tier: "TIER",
      member: "Member",
      nextReward: "NEXT REWARD",
      nextRewardValue: "\u20ac10 off at 500pts",
      multiplier: "MULTIPLIER",
      multiplierValue: "1x",
      inviteEarn: "Invite & Earn",
      viewDashboard: "VIEW DASHBOARD",
      inviteDescription: "Earn 10% commission when colleagues order using your code. Plus commissions up to 5 levels deep.",
      copyCode: "COPY CODE",
      copyLink: "COPY LINK",
      share: "SHARE:",
      accountBenefits: "ACCOUNT BENEFITS",
      benefits: [
        { title: "Order Tracking", desc: "Real-time updates on all your shipments" },
        { title: "Order History", desc: "Full history with reorder capability" },
        { title: "Referral Earnings", desc: "Earn commissions on referrals" },
        { title: "Priority Support", desc: "Account holders get faster responses" },
      ],
    },
    login: {
      title: "Welcome Back",
      subtitle: "Sign in to your ORYN account",
      email: "EMAIL",
      password: "PASSWORD",
      emailPlaceholder: "your@email.com",
      passwordPlaceholder: "Enter your password",
      signingIn: "SIGNING IN...",
      signIn: "SIGN IN",
      noAccount: "Don't have an account?",
      createOne: "Create one",
      continueWithout: "Continue shopping without an account",
      loginFailed: "Login failed",
    },
    register: {
      title: "Create Account",
      subtitle: "Join ORYN for order tracking, exclusive offers, and more",
      firstName: "FIRST NAME *",
      lastName: "LAST NAME *",
      email: "EMAIL *",
      emailPlaceholder: "your@email.com",
      password: "PASSWORD *",
      passwordPlaceholder: "Minimum 8 characters",
      confirmPassword: "CONFIRM PASSWORD *",
      organization: "ORGANIZATION",
      organizationPlaceholder: "Lab, university, company...",
      referralCode: "REFERRAL CODE",
      referralCodePlaceholder: "Optional",
      creating: "CREATING ACCOUNT...",
      createAccount: "CREATE ACCOUNT",
      termsPrefix: "By creating an account, you agree to ORYN's",
      termsOfService: "Terms of Service",
      and: "and",
      privacyPolicy: "Privacy Policy",
      hasAccount: "Already have an account?",
      signIn: "Sign in",
      passwordsMismatch: "Passwords do not match",
      passwordTooShort: "Password must be at least 8 characters",
      registrationFailed: "Registration failed",
    },
    profile: {
      title: "Profile Settings",
      subtitle: "Manage your account information",
      personalInfo: "PERSONAL INFORMATION",
      firstName: "FIRST NAME",
      lastName: "LAST NAME",
      email: "EMAIL",
      emailNotChangeable: "Email cannot be changed",
      referralCode: "REFERRAL CODE",
      saveChanges: "SAVE CHANGES",
      saved: "Saved",
      changePassword: "CHANGE PASSWORD",
      currentPassword: "CURRENT PASSWORD",
      newPassword: "NEW PASSWORD",
      newPasswordPlaceholder: "Minimum 8 characters",
      confirmNewPassword: "CONFIRM NEW PASSWORD",
      updatePassword: "UPDATE PASSWORD",
      passwordUpdated: "Password updated",
      passwordsMismatch: "Passwords do not match",
      passwordTooShort: "Password must be at least 8 characters",
    },
    orders: {
      title: "Your Orders",
      subtitle: "Track and manage all your ORYN orders",
      noOrders: "No orders yet",
      noOrdersDesc: "Your order history will appear here once you make your first purchase.",
      browseProducts: "BROWSE PRODUCTS",
      addedToCart: "ADDED TO CART \u2713",
      reorder: "REORDER",
      items: "items",
      item: "item",
    },
    orderDetail: {
      notFound: "Order not found",
      backToOrders: "Back to orders",
      ordersBreadcrumb: "Orders",
      orderTitle: "Order",
      placedOn: "Placed on",
      print: "PRINT",
      orderTracking: "ORDER TRACKING",
      pending: "Pending",
      processing: "Processing",
      shipped: "Shipped",
      delivered: "Delivered",
      cancelled: "Cancelled",
      orderItems: "ORDER ITEMS",
      qty: "Qty:",
      subtotal: "Subtotal",
      shipping: "Shipping",
      free: "FREE",
      discount: "Discount",
      total: "Total",
      shippingAddress: "SHIPPING ADDRESS",
      shippingNotAvailable: "Shipping details not available",
      payment: "PAYMENT",
      method: "Method",
      status: "Status",
      addedToCart: "ADDED TO CART",
      reorderAll: "REORDER ALL ITEMS",
      needHelp: "Need Help?",
      needHelpDesc: "If you have questions about your order, contact us at",
      },
    referrals: {
      title: "Referral Program",
      subtitle: "Share ORYN with colleagues and earn commissions on every order they place.",
      yourCode: "YOUR REFERRAL CODE",
      copied: "COPIED!",
      copyCode: "COPY CODE",
      yourLink: "YOUR REFERRAL LINK",
      copyLink: "COPY LINK",
      shareVia: "SHARE VIA:",
      whatsapp: "WhatsApp",
      email: "Email",
      directReferrals: "DIRECT REFERRALS",
      networkSize: "NETWORK SIZE",
      totalEarned: "TOTAL EARNED",
      available: "AVAILABLE",
      earningsByLevel: "EARNINGS BY LEVEL",
      levelLabel: "Level",
      commission: "commission",
      ordersLabel: "orders",
      pending: "PENDING",
      approved: "APPROVED",
      paidOut: "PAID OUT",
      yourReferrals: "YOUR REFERRALS",
      commissionHistory: "COMMISSION HISTORY",
      tableOrder: "ORDER",
      tableLevel: "LEVEL",
      tableRate: "RATE",
      tableAmount: "AMOUNT",
      tableStatus: "STATUS",
      tableDate: "DATE",
      howItWorks: "HOW IT WORKS",
      steps: [
        { title: "Share Your Code", desc: "Send your unique referral code or link to colleagues and fellow researchers." },
        { title: "They Order", desc: "When they register with your code and place an order, you earn commission." },
        { title: "Earn Rewards", desc: "Earn 10% on direct referrals, plus commissions on their referrals up to 5 levels deep." },
      ],
      whatsappShareText: "Check out ORYN Peptides — precision research peptides with GMP manufacturing in South Korea. Use my code {code} for tracking on your first order: {link}",
      emailShareSubject: "ORYN Peptides — Precision Research Peptides",
      emailShareBody: "Hi,\n\nI wanted to share ORYN Peptides with you. They produce pharmaceutical-grade research peptides with >99% purity.\n\nUse my referral code: {code}\n\nOr click here: {link}\n\nBest regards",
    },
    wishlist: {
      title: "Wishlist",
      itemsSaved: "items saved",
      itemSaved: "item saved",
      empty: "Your wishlist is empty",
      emptyDesc: "Save products you're interested in for later.",
      browseProducts: "BROWSE PRODUCTS",
      addToCart: "ADD TO CART",
      remove: "REMOVE",
    },
    savedAddresses: {
      title: "SAVED ADDRESSES",
      cancel: "CANCEL",
      addAddress: "+ ADD ADDRESS",
      labelField: "LABEL (e.g. Home, Office)",
      labelPlaceholder: "Home",
      firstName: "FIRST NAME",
      lastName: "LAST NAME",
      address: "ADDRESS",
      city: "CITY",
      postcode: "POSTCODE",
      country: "COUNTRY",
      phone: "PHONE",
      setAsDefault: "Set as default address",
      saveAddress: "SAVE ADDRESS",
      noAddresses: "No saved addresses. Add one for faster checkout.",
      default: "DEFAULT",
      setDefault: "SET DEFAULT",
      remove: "REMOVE",
      countries: {
        GB: "United Kingdom",
        ES: "Spain",
        IE: "Ireland",
        DE: "Germany",
        FR: "France",
        NL: "Netherlands",
      },
    },
  },

  cookie: {
    message:
      "We use cookies to enhance your experience and for essential site functionality. By continuing, you agree to our",
    privacyPolicy: "Privacy Policy",
    acceptAll: "ACCEPT ALL",
    essentialOnly: "ESSENTIAL ONLY",
    essential: "Essential cookies",
    alwaysOn: "always on",
    analyticsLabel: "Analytics cookies",
    marketingLabel: "Marketing cookies",
    savePreferences: "SAVE PREFERENCES",
    manageSettings: "MANAGE",
    hideSettings: "HIDE SETTINGS",
    manageCookies: "Cookie Settings",
  },

  toast: {
    addedToCart: "ADDED TO CART",
  },

  popups: {
    firstVisit: {
      discount: "10%",
      yourFirstOrder: "YOUR FIRST ORDER",
      code: "WELCOME10",
      tagline: "WELCOME TO ORYN",
      title: "Precision Peptide Science",
      description:
        "Biotech lab delivering >99% purity research peptides in precision pen systems. Manufactured in South Korea. Use code WELCOME10 on your first order.",
      benefits: [
        "Free shipping over £150",
        "GMP certified manufacturing",
        "Certificate of Analysis included",
      ],
      shopNow: "SHOP NOW",
      noThanks: "No thanks, I'll browse on my own",
    },
    exitIntent: {
      tagline: "WAIT — BEFORE YOU GO",
      title: "Get 10% Off Your First Order",
      description:
        "Join researchers worldwide using ORYN precision peptides. Enter your email to receive your exclusive discount code.",
      placeholder: "your@email.com",
      claimDiscount: "CLAIM MY 10% DISCOUNT",
      noSpam: "No spam. Unsubscribe anytime.",
      welcomeTitle: "Welcome to ORYN!",
      welcomeMessage:
        "Use code WELCOME10 at checkout for 10% off.",
    },
  },

  payment: {
    processing: "PROCESSING...",
    loadingPayment: "LOADING PAYMENT...",
    completeOrder: "COMPLETE ORDER",
    sslEncrypted: "SSL ENCRYPTED",
    pciCompliant: "PCI COMPLIANT",
    paymentFailed: "Payment failed. Please try again.",
    unexpectedError: "An unexpected error occurred.",
    notCompleted: "Payment was not completed. Please try again.",
    preparingPayment: "Preparing secure payment...",
    paymentNotReady: "Payment system not ready. Please try again.",
    paymentFormLoading: "Payment form is still loading. Please wait a moment.",
    orderCreationFailed: "Payment processed but order creation failed. Please contact support.",
    orPayWith: "OR PAY WITH CARD",
  },

  volumeDiscount: {
    applied: "Volume discount applied:",
    addMore: "Add {count} more {itemWord} for {percent}% OFF",
    title: "VOLUME DISCOUNTS",
    items: "{count}+ items",
    off: "{percent}% OFF",
    active: "ACTIVE",
    unlockMore: "Add {count} more {itemWord} to unlock {percent}% off!",
  },

  frequentlyBought: {
    title: "Frequently Bought Together",
    save: "SAVE 10%",
    bundlePrice: "Bundle price — save {amount} with this combination",
    addAll: "ADD ALL TO CART",
  },

  recentlyViewed: {
    title: "Recently Viewed",
  },

  orderBump: {
    addToOrder: "ADD TO YOUR ORDER",
    yesAddIt: "YES, ADD IT",
  },

  aria: {
    search: "Search",
    account: "Account",
    wishlist: "Wishlist",
    openCart: "Open cart",
    toggleMenu: "Toggle menu",
    mobileMenu: "Mobile menu",
    backToTop: "Back to top",
    cookieConsent: "Cookie consent",
    shareWhatsApp: "Share on WhatsApp",
    shareX: "Share on X",
    shareFacebook: "Share on Facebook",
    copyLink: "Copy link",
    switchLanguage: "Switch language",
    switchRegion: "Switch region",
    breadcrumb: "Breadcrumb",
    dismiss: "Dismiss",
    close: "Close",
    instagram: "Instagram",
    linkedin: "LinkedIn",
    email: "Email",
    researchDisclaimer: "Research disclaimer",
  },

  calculator: {
    interactiveTool: "INTERACTIVE TOOL",
    title: "Peptide Reconstitution Calculator",
    description: "Enter your peptide amount, water volume and desired dose to calculate concentration and injection volume.",
    peptideAmount: "Peptide Amount",
    bacteriostaticWater: "Bacteriostatic Water Volume",
    desiredDose: "Desired Dose Per Injection",
    customAmount: "Custom amount",
    customVolume: "Custom volume",
    customDose: "Custom dose",
    concentration: "Concentration",
    injectionVolume: "Injection Volume",
    insulinSyringe: "Insulin Syringe",
    totalDoses: "Total Doses",
    mcgPerMl: "mcg / mL",
    mlPerDose: "mL per dose",
    unitsIuMarks: "units (IU marks)",
    fromVial: "from vial",
    units: "units",
    overHundredUnits: "> 100 units",
    zeroUnits: "0 units",
    ml: "mL",
    overOneMl: "> 1.0 mL (use larger syringe)",
    zeroMl: "0.000 mL",
    syringeLabel: "Insulin Syringe (100 units = 1 mL)",
    warningNote: "Note:",
    warningText: "The calculated dose exceeds a standard 100-unit insulin syringe (1 mL). Consider adding more bacteriostatic water to increase the volume, which will lower the concentration and reduce the injection volume per dose.",
    enterValues: "Enter valid values above to see your calculated results.",
    quickReference: "Quick Reference",
    unitConversions: "Unit Conversions",
    standardSyringe: "Standard Syringe",
    theFormula: "The Formula",
    mgToMcg: "1 mg = 1,000 mcg",
    mlToUnits: "1 mL = 100 insulin units",
    u100Syringe: "U-100 insulin syringe",
    hundredUnitsOneMl: "100 units = 1.0 mL",
    formulaLine1: "Dose (mcg) / Concentration (mcg/mL)",
    formulaLine2: "= Volume to inject (mL)",
  },

  whyOrynPage: {
    differentiators: [
      { title: "Pre-Dosed Peptide Pens", description: "Unlike competitors selling loose vials requiring reconstitution, ORYN delivers pre-mixed, precision-dosed peptide pens ready for immediate research use. No mixing, no math, no contamination risk.", highlight: "Unique in the UK market" },
      { title: "98%+ HPLC Verified Purity", description: "Every ORYN batch exceeds 98% purity, independently verified via High-Performance Liquid Chromatography and Mass Spectrometry. Most UK competitors offer only 95% or unverified claims.", highlight: "Above industry standard" },
      { title: "GMP Manufacturing in South Korea", description: "All ORYN peptides are manufactured in GMP-certified facilities in South Korea adhering to pharmaceutical production standards. Full batch traceability from raw material to finished product.", highlight: "Pharmaceutical grade" },
      { title: "Free UK Shipping Over \u20ac150", description: "Enjoy complimentary next-day delivery on all UK orders over \u20ac150. Temperature-controlled packaging ensures peptide integrity from our facility to your research lab.", highlight: "Next-day delivery" },
      { title: "Comprehensive COA with Every Order", description: "Every ORYN order ships with a full Certificate of Analysis confirming purity, identity, endotoxin levels, and sterility. No waiting, no requesting \u2014 it is included as standard.", highlight: "Transparency guaranteed" },
      { title: "ISO 7 Cleanroom Production", description: "ORYN peptides are filled and sealed in ISO 7 classified cleanrooms with continuous environmental monitoring. This exceeds the standards of most UK peptide suppliers.", highlight: "Hospital-grade environment" },
    ],
    comparisonFeatures: [
      { feature: "Delivery Format", oryn: "Pre-dosed Pen System", competitor: "Loose Vials (reconstitution required)" },
      { feature: "Purity Standard", oryn: "98%+ HPLC & MS Verified", competitor: "95%+ (often unverified)" },
      { feature: "Testing Protocol", oryn: "HPLC + Mass Spectrometry + Endotoxin", competitor: "Basic HPLC only" },
      { feature: "UK Shipping", oryn: "Free over \u20ac150 (next-day)", competitor: "Varies (\u20ac5-\u20ac15)" },
      { feature: "COA Included", oryn: "Every batch, every order", competitor: "On request (if available)" },
      { feature: "Production Facility", oryn: "ISO 7 Cleanroom, GMP Certified", competitor: "Varies (often undisclosed)" },
      { feature: "Sterility", oryn: "0.22\u03bcm Filter + Gamma Ray", competitor: "Filtration only" },
      { feature: "Shelf Life", oryn: "24 months (sealed)", competitor: "6-12 months" },
      { feature: "Customer Support", oryn: "Dedicated research team", competitor: "Email only" },
      { feature: "Product Range", oryn: "10 peptides across 3 delivery systems", competitor: "5-8 peptides, vials only" },
    ],
    stats: [
      { value: "10", label: "Research Peptides", sublabel: "across 3 delivery systems" },
      { value: "98%+", label: "Verified Purity", sublabel: "HPLC + MS tested" },
      { value: "ISO 7", label: "Cleanroom Standard", sublabel: "pharmaceutical grade" },
      { value: "15+", label: "UK Cities Served", sublabel: "next-day delivery" },
      { value: "40+", label: "London Areas", sublabel: "same-day dispatch available" },
      { value: "24", label: "Month Shelf Life", sublabel: "sealed, refrigerated" },
    ],
    tableOryn: "ORYN",
    tableCompetitors: "TYPICAL UK COMPETITORS",
    feature: "Feature",
    viewProducts: "VIEW OUR PRODUCTS",
    ourNumbers: "OUR NUMBERS",
    comparisonTitle: "How ORYN Compares",
    comparisonSubtitle: "See how ORYN stacks up against typical UK peptide suppliers.",
  },

  wholesalePage: {
    breadcrumbHome: "HOME",
    breadcrumbWholesale: "WHOLESALE",
    heroBulkOrders: "BULK ORDERS",
    heroTitle1: "Wholesale & Bulk",
    heroTitle2: "Peptide Orders",
    heroDescription: "ORYN supplies research-grade peptide pen systems to universities, laboratories, clinics, and distributors across the UK and Europe. Volume discounts from 5% to 15% — with custom pricing for large-scale contracts.",
    statMinOrderLabel: "Min. Order",
    statMinOrderValue: "3 Units",
    statMaxDiscountLabel: "Max Discount",
    statMaxDiscountValue: "15% Off",
    statPurityLabel: "Purity",
    statPurityValue: ">99%",
    statDispatchLabel: "UK Dispatch",
    statDispatchValue: "Same Day",
    tiersLabel: "VOLUME PRICING",
    tiersTitle1: "Discount",
    tiersTitle2: "Tiers",
    tiersDescription: "Mix and match any products across our entire range. Discounts apply automatically based on total unit count.",
    tierMostPopular: "MOST POPULAR",
    tierOffRetail: "OFF RETAIL",
    tierPricing: "PRICING",
    tierLabelStarter: "STARTER",
    tierLabelProfessional: "PROFESSIONAL",
    tierLabelEnterprise: "ENTERPRISE",
    tierLabelCustom: "CUSTOM QUOTE",
    benefitsLabel: "WHOLESALE ADVANTAGES",
    benefitsTitle1: "Why Partner With",
    benefitsTitle2: "ORYN",
    benefitsDescription: "Beyond volume pricing, ORYN wholesale accounts unlock a suite of services designed for professional buyers.",
    benefitAccountManagerTitle: "Dedicated Account Manager",
    benefitAccountManagerDesc: "A single point of contact for orders, pricing, and technical queries. Your account manager understands your needs.",
    benefitShippingTitle: "Priority Shipping",
    benefitShippingDesc: "Wholesale orders ship same day when placed before 2pm. Temperature-controlled packaging included at no extra cost.",
    benefitVolumePricingTitle: "Volume Pricing",
    benefitVolumePricingDesc: "Automatic discounts from 5% to 15% based on order quantity. Custom pricing available for ongoing contracts.",
    benefitCOATitle: "COA Included",
    benefitCOADesc: "Every wholesale shipment includes batch-specific Certificates of Analysis. Full traceability from synthesis to delivery.",
    benefitCustomLabellingTitle: "Custom Labelling",
    benefitCustomLabellingDesc: "White-label and custom packaging available for distributors and clinics. Minimum 50-unit orders for custom labels.",
    benefitTechSupportTitle: "Technical Support",
    benefitTechSupportDesc: "Access to our peptide science team for protocol guidance, storage recommendations, and product specifications.",
    whoWeServeLabel: "OUR CUSTOMERS",
    whoWeServeTitle1: "Who We",
    whoWeServeTitle2: "Serve",
    whoWeServeDescription: "ORYN wholesale peptides are trusted by research professionals and organisations across the UK and Europe.",
    customerUniversitiesTitle: "Universities & Academia",
    customerUniversitiesDesc: "Research departments and postgraduate programmes studying peptide biology, pharmacology, and regenerative medicine.",
    customerResearchLabsTitle: "Research Laboratories",
    customerResearchLabsDesc: "Private and public research labs conducting in-vitro and in-vivo studies with research-grade peptide compounds.",
    customerPharmaTitle: "Pharmaceutical Companies",
    customerPharmaDesc: "Pharmaceutical R&D divisions using reference-standard peptides for drug development and analytical comparison studies.",
    customerClinicsTitle: "Clinics & Medical Practices",
    customerClinicsDesc: "Integrative medicine clinics and practitioners requiring consistent, high-purity peptide supply for clinical research.",
    customerDistributorsTitle: "Distributors & Resellers",
    customerDistributorsDesc: "Wholesale partners distributing ORYN products across the UK and Europe. White-label options available.",
    customerBiotechTitle: "Biotech Startups",
    customerBiotechDesc: "Emerging biotech companies needing reliable peptide supply for product development and proof-of-concept studies.",
    productsLabel: "WHOLESALE PRICING EXAMPLES",
    productsTitle1: "Products at",
    productsTitle2: "Wholesale Prices",
    productsDescription: "See how volume pricing reduces your cost per unit. All ORYN products are eligible for wholesale discounts.",
    productRetailLabel: "RETAIL",
    productViewAll: "VIEW ALL {count} PRODUCTS",
    faqLabel: "FAQ",
    faqTitle1: "Wholesale",
    faqTitle2: "FAQ",
    faqDescription: "Common questions about our wholesale programme, pricing, and account setup.",
    ctaTitle: "Ready to Order in Bulk?",
    ctaDescription: "Contact our wholesale team to set up your account, discuss custom pricing, and start saving on research-grade peptide pens.",
    ctaEnquiry: "WHOLESALE ENQUIRY",
    ctaBrowseProducts: "BROWSE PRODUCTS",
  },

  qualityPage: {
    // Hero
    breadcrumbHome: "HOME",
    breadcrumbQuality: "QUALITY & TESTING",
    heroBadge: "THIRD-PARTY VERIFIED",
    heroTitle1: "Transparency in",
    heroTitle2: "Every Batch",
    heroDescription:
      "Every ORYN peptide is independently tested by ISO-accredited laboratories. We publish Certificates of Analysis for every product because we believe you deserve to see exactly what you're getting — purity, identity, and sterility, verified by science, not marketing.",
    statPurityLabel: "BATCH PURITY",
    statLabsLabel: "INDEPENDENT LABS",
    statTestsLabel: "TESTS PER BATCH",
    statTraceabilityLabel: "TRACEABILITY",

    // Testing Process section
    processLabel: "OUR PROCESS",
    processTitle1: "From Synthesis to",
    processTitle2: "Certificate",
    processDescription:
      "A rigorous four-step quality assurance pipeline ensures every ORYN peptide meets the highest analytical standards before release.",
    testingSteps: [
      {
        title: "Peptide Synthesis",
        description:
          "Solid-phase peptide synthesis (SPPS) in GMP-certified facilities in South Korea under ISO 9001 quality management.",
      },
      {
        title: "HPLC Analysis",
        description:
          "High-Performance Liquid Chromatography separates and quantifies peptide purity, confirming >98% for every batch.",
      },
      {
        title: "Mass Spectrometry",
        description:
          "LC-MS/MS identity confirmation verifies exact molecular weight and amino acid sequence integrity.",
      },
      {
        title: "Certificate Generation",
        description:
          "Independent lab issues batch-specific Certificate of Analysis with full analytical data and pass/fail criteria.",
      },
    ],

    // Quality Standards section
    certificationsLabel: "CERTIFICATIONS",
    standardsTitle1: "Quality Standards That",
    standardsTitle2: "Define Us",
    standardsDescription:
      "Our manufacturing and testing infrastructure meets the most demanding pharmaceutical quality benchmarks in the industry.",
    qualityStandards: [
      {
        title: "GMP Manufacturing",
        description:
          "Good Manufacturing Practice certified production facilities across the EU, ensuring pharmaceutical-grade consistency.",
      },
      {
        title: "ISO 9001 Quality",
        description:
          "Internationally recognised quality management system governing every step from raw materials to finished product.",
      },
      {
        title: "ISO 7 Cleanroom",
        description:
          "Sterile filling performed in ISO 7 classified cleanrooms with HEPA filtration and continuous environmental monitoring.",
      },
      {
        title: "HPLC Verified Purity",
        description:
          "Every batch verified by independent HPLC testing. Most batches exceed 99% purity — among the highest in the UK market.",
      },
    ],

    // COA Grid section
    coaLabel: "CERTIFICATES OF ANALYSIS",
    coaTitle1: "COA for Every",
    coaTitle2: "Product",
    coaDescription:
      "Each of our peptide products is independently verified. Below are the latest batch test results — updated with every new production run.",
    passLabel: "PASS",
    purityLabel: "PURITY",
    batchLabel: "BATCH",
    testedLabel: "TESTED",
    methodLabel: "METHOD",
    testedDate: "Feb 2026",
    viewCoa: "VIEW COA",

    // Third-Party Testing section
    independentLabel: "INDEPENDENT VERIFICATION",
    partnersTitle1: "Third-Party Testing",
    partnersTitle2: "Partners",
    partnersP1:
      "ORYN does not test its own products. Every Certificate of Analysis is generated by independent, ISO 17025-accredited analytical laboratories that have no financial relationship with our manufacturing operation.",
    partnersP2:
      "This separation ensures unbiased, scientifically rigorous verification of purity, identity, and sterility. Our testing partners use validated analytical methods including reverse-phase HPLC, LC-MS/MS, LAL endotoxin testing, and membrane filtration sterility testing.",
    partnersBullets: [
      "ISO 17025 accredited analytical laboratories",
      "No financial ties to ORYN manufacturing",
      "Validated HPLC and mass spectrometry methods",
      "Endotoxin and sterility verification",
      "Full batch traceability from synthesis to certificate",
    ],
    analyticalMethodsLabel: "ANALYTICAL METHODS",
    analyticalMethods: [
      {
        method: "HPLC Purity Testing",
        description:
          "Reverse-phase C18 column, UV detection at 220nm. Quantifies peptide purity and detects impurities.",
        standard: "USP <621>",
      },
      {
        method: "Mass Spectrometry (LC-MS)",
        description:
          "Electrospray ionisation with high-resolution mass detection. Confirms molecular identity.",
        standard: "ISO 13528",
      },
      {
        method: "Endotoxin Testing",
        description:
          "Limulus Amebocyte Lysate (LAL) kinetic turbidimetric assay. Ensures bacterial endotoxin levels below limits.",
        standard: "USP <85>",
      },
      {
        method: "Sterility Testing",
        description:
          "Membrane filtration method with 14-day incubation in TSB and FTM media.",
        standard: "USP <71>",
      },
    ],

    // COA Contents section
    understandingLabel: "UNDERSTANDING YOUR COA",
    coaContentsTitle1: "What Every COA",
    coaContentsTitle2: "Contains",
    coaContentsDescription:
      "Each ORYN Certificate of Analysis provides complete analytical transparency. Here is what you will find in every document.",
    coaItems: [
      {
        title: "Peptide Identity",
        details:
          "Compound name, molecular formula, molecular weight, amino acid sequence, and CAS number.",
      },
      {
        title: "HPLC Purity Data",
        details:
          "Chromatogram, retention time, peak area percentage, and impurity profile with acceptance criteria.",
      },
      {
        title: "Mass Spectrum",
        details:
          "LC-MS identity confirmation showing observed vs. theoretical molecular weight and charge states.",
      },
      {
        title: "Endotoxin Results",
        details:
          "LAL test results in EU/mL with specification limits. All ORYN products test below 0.5 EU/mL.",
      },
      {
        title: "Sterility Report",
        details:
          "14-day incubation results in TSB and FTM media confirming no microbial growth detected.",
      },
      {
        title: "Batch Information",
        details:
          "Unique batch number, manufacturing date, test date, expiry date, and analyst identification.",
      },
    ],

    // FAQ section
    faqLabel: "FAQ",
    faqTitle1: "Quality & Testing",
    faqTitle2: "FAQ",
    faqDescription:
      "Common questions about our quality assurance process, peptide purity testing, and Certificates of Analysis.",

    // CTA section
    ctaTitle: "Quality You Can Verify",
    ctaDescription:
      "Every ORYN peptide ships with a batch number you can trace back to an independent Certificate of Analysis. See the science for yourself.",
    ctaBrowseProducts: "BROWSE PRODUCTS",
    ctaRequestCoa: "REQUEST A COA",
  },

  share: {
    label: "SHARE",
  },

  socialProof: {
    from: "from",
    purchased: "purchased",
    minAgo: "{min} min ago",
  },

  flashSale: {
    flash: "Flash Sale — Use code FLASH15 for 15% off all peptide pens",
    weekend: "Weekend Special — Use code WEEKEND10 for 10% off",
  },

  shippingPage: {
    // Hero
    heroTagline: "TEMPERATURE-CONTROLLED",
    heroTitle1: "Shipping &",
    heroTitle2: "Delivery",
    heroDescription:
      "Every ORYN order is dispatched same day in temperature-controlled packaging, ensuring your peptide pens arrive in perfect condition. Free UK shipping on orders over \u20ac{threshold}.",
    breadcrumbHome: "HOME",
    breadcrumbShipping: "SHIPPING & DELIVERY",
    // Hero trust bar stats
    statUkDeliveryLabel: "UK Delivery",
    statUkDeliveryValue: "2-4 Days",
    statDispatchLabel: "Dispatch",
    statDispatchValue: "Same Day",
    statFreeOverLabel: "Free Over",
    statPackagingLabel: "Packaging",
    statPackagingValue: "2-8\u00b0C",
    // UK Delivery Zones section
    ukZonesSectionLabel: "UK DELIVERY ZONES",
    ukZonesSectionTitle1: "Delivery Times by",
    ukZonesSectionTitle2: "Region",
    ukZonesSectionDescription:
      "All orders placed before 2pm GMT are dispatched the same business day. Delivery times shown are business days from dispatch.",
    ukDeliveryNote:
      "Delivery estimates are for standard shipping. Express options are available at checkout for faster delivery. Weekend and bank holiday orders are dispatched on the next business day.",
    ukZones: [
      { region: "London & South East", days: "1 \u2013 2 days" },
      { region: "Midlands & East Anglia", days: "2 \u2013 3 days" },
      { region: "North England", days: "2 \u2013 3 days" },
      { region: "Scotland (Lowlands)", days: "3 \u2013 4 days" },
      { region: "Wales", days: "2 \u2013 3 days" },
      { region: "Northern Ireland", days: "3 \u2013 4 days" },
      { region: "Highlands & Islands", days: "4 \u2013 5 days" },
    ],
    // Cold chain / Packaging section
    coldChainSectionLabel: "COLD CHAIN LOGISTICS",
    packagingTitle1: "Temperature-Controlled",
    packagingTitle2: "Packaging",
    packagingDescription:
      "Peptides require refrigerated storage at 2-8\u00b0C. Our cold chain packaging ensures your order maintains optimal temperature throughout transit \u2014 from our climate-controlled warehouse to your door.",
    packagingBullets: [
      "Maintains 2-8\u00b0C for up to 48 hours in transit",
      "Non-toxic gel ice packs, pre-cooled to 2\u00b0C",
      "Multi-layer insulated shipping containers",
      "Extra protection during summer months",
      "Custom foam inserts prevent pen movement",
    ],
    coldChainFeatures: [
      {
        title: "Insulated Shipping Boxes",
        description:
          "Multi-layer insulated containers maintain 2-8\u00b0C internal temperature for up to 48 hours, protecting peptide integrity from warehouse to doorstep.",
      },
      {
        title: "Gel Ice Packs",
        description:
          "Non-toxic phase-change gel packs pre-cooled to 2\u00b0C provide sustained cooling throughout transit without risk of freezing the product.",
      },
      {
        title: "Seasonal Adjustments",
        description:
          "During summer months (June-September), we add extra ice packs and upgraded insulation. In extreme heat, we switch to express delivery to minimise transit time.",
      },
      {
        title: "Temperature Monitoring",
        description:
          "Wholesale orders of 25+ units include temperature indicator strips that confirm the shipment remained within the 2-8\u00b0C range throughout delivery.",
      },
    ],
    // European Delivery section
    euSectionLabel: "INTERNATIONAL SHIPPING",
    euSectionTitle1: "European",
    euSectionTitle2: "Delivery",
    euSectionDescription:
      "ORYN ships to 15+ European countries with full tracking and temperature-controlled packaging.",
    euCountriesHeader: "EU COUNTRIES WE SHIP TO",
    euCountriesNotListed:
      "Not listed? Contact us \u2014 we may be able to arrange shipping to your country.",
    euDeliveryTimeTitle: "Delivery Time",
    euDeliveryTimeValue: "3 \u2013 7 days",
    euDeliveryTimeDescription:
      "Business days from dispatch. Western Europe typically 3-5 days, Eastern Europe 5-7 days.",
    euShippingCostTitle: "Shipping Cost",
    euShippingCostValue: "Flat Rate",
    euShippingCostDescription:
      "Flat-rate shipping to all European destinations. Calculated at checkout based on destination country and order weight.",
    euCustomsTitle: "Customs & Duties",
    euCustomsValue: "Buyer's Responsibility",
    euCustomsDescription:
      "International orders may be subject to customs duties and import taxes. These charges are the responsibility of the customer and vary by country.",
    euTrackingTitle: "Tracking",
    euTrackingValue: "Full Tracking",
    euTrackingDescription:
      "All international shipments include end-to-end tracking with real-time status updates via email.",
    // Free Shipping section
    freeShippingSectionLabel: "FREE SHIPPING",
    freeShippingTitle1: "Free UK Shipping Over",
    freeShippingTitle2: "\u20ac{threshold}",
    freeShippingDescription:
      "Orders over \u20ac{threshold} qualify for free standard UK delivery. Track your progress in your cart.",
    freeShippingExampleLabel: "EXAMPLE: \u20ac120 ORDER",
    freeShippingExampleAway: "\u20ac30 away from free shipping",
    // Discreet / Tracking / Returns cards
    discreetTitle: "Discreet Packaging",
    discreetDescription:
      "All ORYN orders ship in plain, unmarked boxes. No product names, no branding, and no descriptions are visible on the exterior packaging or shipping label.",
    discreetItems: [
      "Plain outer box",
      "No visible branding",
      "Generic sender name",
      "No product descriptions",
    ],
    trackingTitle: "Order Tracking",
    trackingDescription:
      "Every order receives a tracking number within 2 hours of dispatch. Follow your shipment in real time from our warehouse to your door.",
    trackingItems: [
      "Tracking email within 2 hours",
      "Real-time status updates",
      "Estimated delivery window",
      "Delivery confirmation",
    ],
    returnsTitle: "Returns & Refunds",
    returnsDescription:
      "We accept returns within 30 days of delivery. Unopened products in original packaging qualify for a full refund. Damaged or incorrect items must be reported within 48 hours with photos for a free replacement or refund.",
    returnsItems: [
      "30-day return window from delivery",
      "Full refund for unopened products",
      "48-hour reporting for damaged/incorrect items",
      "Free return shipping for our errors",
      "Refunds processed in 5-10 business days",
    ],
    // FAQ section
    faqSectionLabel: "FAQ",
    faqSectionTitle1: "Shipping",
    faqSectionTitle2: "FAQ",
    faqSectionDescription:
      "Common questions about peptide delivery, packaging, and shipping options.",
    // CTA section
    ctaTitle: "Order Today, Receive This Week",
    ctaDescription:
      "Same-day dispatch before 2pm. Temperature-controlled delivery. Free UK shipping over \u20ac{threshold}.",
    ctaShopNow: "SHOP NOW",
    ctaContactUs: "CONTACT US",
  },

  termsPage: {
    tagline: "LEGAL",
    title: "Terms of Service",
    lastUpdated: "Last updated: March 2026",
    sections: [
      {
        heading: "1. Acceptance of Terms",
        content:
          "By accessing and using the ORYN Peptide Labs website and services, you agree to be bound by these Terms of Service. If you do not agree to these terms, please do not use our services.",
      },
      {
        heading: "2. Research Use Only",
        content:
          "All products sold by ORYN Peptide Labs are intended for laboratory and research purposes only. Products are not intended for human or animal consumption. By purchasing, you confirm you are acquiring products for legitimate research purposes and comply with all applicable laws and regulations in your jurisdiction.",
      },
      {
        heading: "3. Eligibility",
        content:
          "You must be at least 18 years old and represent a legitimate research organization, laboratory, or academic institution to purchase products. ORYN reserves the right to request verification of research credentials.",
      },
      {
        heading: "4. Orders and Payment",
        content:
          "All prices are listed in EUR. Payment is processed securely. ORYN reserves the right to cancel orders that appear fraudulent or violate these terms. For wholesale orders: 50% upfront payment, 50% before shipping.",
      },
      {
        heading: "5. Shipping and Delivery",
        content:
          "Products are shipped with cold chain handling where required. Delivery times are estimates. ORYN is not responsible for delays caused by customs, carriers, or force majeure events.",
      },
      {
        heading: "6. Returns and Refunds",
        content:
          "Returns are accepted within 30 days of delivery. Unopened products in original packaging are eligible for a full refund. Damaged or incorrect items must be reported within 48 hours of delivery with photographic evidence; we will arrange a free replacement or full refund. Refunds are processed within 5-10 business days after we receive the returned item. Return shipping costs are the responsibility of the customer unless the return is due to our error.",
      },
      {
        heading: "7. Intellectual Property",
        content:
          "All content, branding, and materials on this website are the property of ORYN Peptide Labs. Unauthorized reproduction is prohibited.",
      },
      {
        heading: "8. Limitation of Liability",
        content:
          "ORYN Peptide Labs provides products as-is for research purposes. We make no warranties regarding fitness for any particular purpose beyond stated specifications. Our liability is limited to the purchase price of the product.",
      },
      {
        heading: "9. Contact",
        content: "For questions regarding these terms, contact us at legal@orynlabs.com.",
      },
    ],
  },

  privacyPage: {
    tagline: "LEGAL",
    title: "Privacy Policy",
    lastUpdated: "Last updated: March 2026",
    sections: [
      {
        heading: "1. Information We Collect",
        content:
          "We collect information you provide directly: name, email, organization, shipping address, and payment details when you create an account or place an order.",
      },
      {
        heading: "2. How We Use Your Information",
        content:
          "Your information is used to: process orders, communicate about your account, provide customer support, send order updates, and comply with legal obligations. We do not sell your personal data.",
      },
      {
        heading: "3. Data Protection",
        content:
          "ORYN Peptide Labs complies with GDPR and applicable European data protection regulations. Your data is stored securely with encryption at rest and in transit.",
      },
      {
        heading: "4. Your Rights",
        content:
          "Under GDPR, you have the right to: access your data, rectify inaccuracies, request deletion, restrict processing, data portability, and object to processing. Contact privacy@orynlabs.com to exercise these rights.",
      },
      {
        heading: "5. Cookies",
        content:
          "We use essential cookies for site functionality and analytics cookies to improve our service. You can manage cookie preferences in your browser settings.",
      },
      {
        heading: "6. Contact",
        content: "Data Protection Officer: privacy@orynlabs.com",
      },
    ],
  },

  disclaimerPage: {
    tagline: "LEGAL",
    title: "Research Disclaimer",
    lastUpdated: "Last updated: March 2026",
    alertTitle: "Important Notice",
    alertContent:
      "All products sold by ORYN Peptide Labs are strictly intended for laboratory research and scientific study. They are NOT intended for human or veterinary use, food additives, drugs, cosmetics, household chemicals, or any other form of consumption.",
    buyerResponsibilityIntro:
      "By purchasing from ORYN Peptide Labs, the buyer acknowledges and agrees that:",
    buyerResponsibilityItems: [
      "Products will be used solely for legitimate research purposes",
      "The buyer is familiar with and compliant with all applicable regulations",
      "Products will not be resold for human consumption",
      "The buyer assumes all responsibility for proper handling and use",
      "The buyer is of legal age and represents a legitimate research entity",
    ],
    sections: [
      {
        heading: "Research Use Statement",
        content:
          "ORYN Peptide Labs manufactures and distributes peptide compounds exclusively for in-vitro research, laboratory experimentation, and scientific investigation. All compounds are supplied as research chemicals with accompanying Certificates of Analysis (COA) for batch verification.",
      },
      {
        heading: "Buyer Responsibility",
        content: "",
      },
      {
        heading: "No Medical Claims",
        content:
          "ORYN Peptide Labs makes no claims regarding the therapeutic, diagnostic, or preventive properties of any product. Product descriptions reference published scientific literature for informational purposes only and do not constitute medical advice or claims of efficacy.",
      },
      {
        heading: "Quality Assurance",
        content:
          "All ORYN products are manufactured in GMP-certified, ISO Class 7 cleanroom facilities with third-party analytical validation. Purity levels exceed 99% as verified by HPLC analysis. Full batch documentation including COA, stability data, and DMF documentation is available upon request.",
      },
      {
        heading: "Regulatory Compliance",
        content:
          "It is the buyer's responsibility to ensure compliance with all local, national, and international regulations regarding the purchase, import, possession, and use of research peptides. ORYN Peptide Labs operates under European regulatory frameworks.",
      },
    ],
  },

  compare: {
    title: "Compare Products",
    customTitle: "Custom",
    compare: "COMPARE",
    comparing: "COMPARING",
    compareNow: "COMPARE",
    clearAll: "CLEAR ALL",
    selectMore: "Select 1 more",
    remove: "Remove",
    productsSelected: "products selected",
    emptyMessage: "Select at least 2 products from the catalog to compare them side by side.",
  },
};

export default en;
